...
bool		print_ml_cost_features = false;
bool		print_nn_cost_features = false;
			...
			ExplainState *es)
{
		...
		planstate->instrument && planstate->instrument->nloops > 0)
	{
		...
		if (es->format == EXPLAIN_FORMAT_TEXT)
		{
			if (es->timing)
				appendStringInfo(es->str,
								 " (actual time=%.8f..%.8f rows=%.0f loops=%.0f)",
		...
		else
		{
			...
			ExplainPropertyFloat("Actual Rows", NULL, rows, 8, es);
			ExplainPropertyFloat("Actual Loops", NULL, nloops, 8, es);
		}
	}
	else if (es->analyze)
	{
		...
		else
		{
			if (es->timing)
			{
				ExplainPropertyFloat("Actual Startup Time", "ms", 0.0, 8, es);
				ExplainPropertyFloat("Actual Total Time", "ms", 0.0, 8, es);
			}
	...
	if (print_ml_cost_features)
	{
		...
		/* Added by hanjun */
		ExplainPropertyFloat("ml_node_startup_cost", NULL, plan->ml_node_startup_cost, 8, es);
		ExplainPropertyFloat("ml_node_total_cost", NULL, plan->ml_node_total_cost, 8, es);
		ExplainPropertyFloat("ml_node_startup_std_cost", NULL, plan->ml_node_startup_std_cost, 8, es);
		ExplainPropertyFloat("ml_node_total_std_cost", NULL, plan->ml_node_total_std_cost, 8, es);
		
		ExplainPropertyFloat("ml_agg_startup_cost", NULL, plan->ml_agg_startup_cost, 8, es);
		ExplainPropertyFloat("ml_agg_total_cost", NULL, plan->ml_agg_total_cost, 8, es);
		ExplainPropertyFloat("ml_agg_startup_std_cost", NULL, plan->ml_agg_startup_std_cost, 8, es);
		ExplainPropertyFloat("ml_agg_total_std_cost", NULL, plan->ml_agg_total_std_cost, 8, es);
		
		ExplainPropertyFloat("ml_inner_run_cost", NULL, plan->ml_inner_run_cost, 8, es);
		ExplainPropertyFloat("ml_inner_rescan_run_cost", NULL, plan->ml_inner_rescan_run_cost, 8, es);
		ExplainPropertyFloat("ml_inner_rescan_start_cost", NULL, plan->ml_inner_rescan_start_cost, 8, es);
		
		ExplainPropertyFloat("ml_sort_left_cost", NULL, plan->ml_sort_left_cost, 8, es);
		ExplainPropertyFloat("ml_sort_left_std_cost", NULL, plan->ml_sort_left_std_cost, 8, es);
		ExplainPropertyFloat("ml_sort_right_cost", NULL, plan->ml_sort_right_cost, 8, es);
		ExplainPropertyFloat("ml_sort_right_std_cost", NULL, plan->ml_sort_right_std_cost, 8, es);
	}

	if (print_nn_cost_features) {
		ExplainPropertyBool("nn_sort_left", plan->nn_sort_left, es);
		ExplainPropertyBool("nn_sort_right", plan->nn_sort_right, es);

		ExplainPropertyFloat("nn_restrict_qual_cost_startup", NULL, plan->nn_restrict_qual_cost_startup, 8, es);
		ExplainPropertyFloat("nn_pathtarget_cost_startup", NULL, plan->nn_pathtarget_cost_startup, 8, es);
		ExplainPropertyFloat("nn_pathtarget_cost_per_tuple", NULL, plan->nn_pathtarget_cost_per_tuple, 8, es);

		ExplainPropertyFloat("nn_outer_path_rows", NULL, plan->nn_outer_path_rows, 8, es);
		ExplainPropertyFloat("nn_inner_path_rows", NULL, plan->nn_inner_path_rows, 8, es);
		ExplainPropertyFloat("nn_path_rows", NULL, plan->nn_path_rows, 8, es);
		ExplainPropertyFloat("nn_per_tuple", NULL, plan->nn_per_tuple, 8, es);
		ExplainPropertyFloat("nn_outer_matched_rows", NULL, plan->nn_outer_matched_rows, 8, es);
		ExplainPropertyFloat("nn_outer_unmatched_rows", NULL, plan->nn_outer_unmatched_rows, 8, es);
		ExplainPropertyFloat("nn_inner_scan_frac", NULL, plan->nn_inner_scan_frac, 8, es);
		ExplainPropertyFloat("nn_ntuples", NULL, plan->nn_ntuples, 8, es);
		ExplainPropertyInteger("nn_parallel_worker", NULL, plan->nn_parallel_worker, es);
		ExplainPropertyInteger("nn_jointype", NULL, plan->nn_jointype, es);
		ExplainPropertyFloat("nn_match_count", NULL, plan->nn_match_count, 8, es);
		ExplainPropertyBool("nn_inner_unique", plan->nn_inner_unique, es);
		ExplainPropertyBool("nn_has_indexed_join_quals", plan->nn_has_indexed_join_quals, es);
		ExplainPropertyFloat("nn_outer_rows", NULL, plan->nn_outer_rows, 8, es);
		ExplainPropertyFloat("nn_inner_rows", NULL, plan->nn_inner_rows, 8, es);
		ExplainPropertyFloat("nn_outer_skip_rows", NULL, plan->nn_outer_skip_rows, 8, es);
		ExplainPropertyFloat("nn_inner_skip_rows", NULL, plan->nn_inner_skip_rows, 8, es);
		
		ExplainPropertyFloat("nn_rescanratio", NULL, plan->nn_rescanratio, 8, es);
		ExplainPropertyFloat("nn_outerstartsel", NULL, plan->nn_outerstartsel, 8, es);
		ExplainPropertyFloat("nn_outerendsel", NULL, plan->nn_outerendsel, 8, es);
		ExplainPropertyFloat("nn_innerstartsel", NULL, plan->nn_innerstartsel, 8, es);
		ExplainPropertyFloat("nn_innerendsel", NULL, plan->nn_innerendsel, 8, es);

		ExplainPropertyFloat("nn_merge_qual_cost_startup", NULL, plan->nn_merge_qual_cost_startup, 8, es);
		ExplainPropertyFloat("nn_merge_qual_cost_per_tuple", NULL, plan->nn_merge_qual_cost_per_tuple, 8, es);
		ExplainPropertyFloat("nn_qp_qual_cost_startup", NULL, plan->nn_qp_qual_cost_startup, 8, es);
		ExplainPropertyFloat("nn_qp_qual_cost_per_tuple", NULL, plan->nn_qp_qual_cost_per_tuple, 8, es);
		ExplainPropertyFloat("nn_mergejointuples", NULL, plan->nn_mergejointuples, 8, es);
		ExplainPropertyFloat("nn_rescannedtuples", NULL, plan->nn_rescannedtuples, 8, es);
		ExplainPropertyBool("nn_materialize_inner", plan->nn_materialize_inner, es);
		ExplainPropertyBool("nn_skip_mark_restore", plan->nn_skip_mark_restore, es);
		ExplainPropertyInteger("nn_numbuckets", NULL, plan->nn_numbuckets, es);
		ExplainPropertyInteger("nn_numbatches", NULL, plan->nn_numbatches, es);
		ExplainPropertyInteger("nn_num_hashclauses", NULL, plan->nn_num_hashclauses, es);
		ExplainPropertyFloat("nn_outerpages", NULL, plan->nn_outerpages, 8, es);
		ExplainPropertyFloat("nn_innerpages", NULL, plan->nn_innerpages, 8, es);
		ExplainPropertyFloat("nn_inner_path_rows_total", NULL, plan->nn_inner_path_rows_total, 8, es);
		ExplainPropertyFloat("nn_hashjointuples", NULL, plan->nn_hashjointuples, 8, es);
		ExplainPropertyFloat("nn_virtualbuckets", NULL, plan->nn_virtualbuckets, 8, es);
		ExplainPropertyFloat("nn_innerbucketsize", NULL, plan->nn_innerbucketsize, 8, es);
		ExplainPropertyFloat("nn_innermcvfreq", NULL, plan->nn_innermcvfreq, 8, es);
		ExplainPropertyFloat("nn_hash_qual_cost_startup", NULL, plan->nn_hash_qual_cost_startup, 8, es);
		ExplainPropertyFloat("nn_hash_qual_cost_per_tuple", NULL, plan->nn_hash_qual_cost_per_tuple, 8, es);
		ExplainPropertyFloat("nn_spc_seq_page_cost", NULL, plan->nn_spc_seq_page_cost, 8, es);
		ExplainPropertyInteger("nn_pages", NULL, plan->nn_pages, es);
		ExplainPropertyFloat("nn_spc_random_page_cost", NULL, plan->nn_spc_random_page_cost, 8, es);
		ExplainPropertyFloat("nn_comparison_cost", NULL, plan->nn_comparison_cost, 8, es);
		ExplainPropertyFloat("nn_parallel_tuple_cost", NULL, plan->nn_parallel_tuple_cost, 8, es);
		ExplainPropertyInteger("nn_num_workers", NULL, plan->nn_num_workers, es);
		
		ExplainPropertyFloat("nn_indexStartupCost", NULL, plan->nn_indexStartupCost, 8, es);
		ExplainPropertyFloat("nn_indexTotalCost", NULL, plan->nn_indexTotalCost, 8, es);
		
		ExplainPropertyFloat("nn_indexselectivity", NULL, plan->nn_indexselectivity, 8, es);
		ExplainPropertyFloat("nn_indexcorrelation", NULL, plan->nn_indexcorrelation, 8, es);
		ExplainPropertyFloat("nn_indexpage", NULL, plan->nn_indexpage, 8, es);
		ExplainPropertyInteger("nn_loopcount", NULL, plan->nn_loopcount, es);
		ExplainPropertyBool("nn_indexonly", plan->nn_indexonly, es);
		ExplainPropertyBool("nn_partial_path", plan->nn_partial_path, es);
		ExplainPropertyFloat("nn_selectivity", NULL, plan->nn_selectivity, 8, es);
		ExplainPropertyBool("nn_iscurrentof", plan->nn_iscurrentof, es);
		ExplainPropertyFloat("nn_scan_qual_cost_startup", NULL, plan->nn_scan_qual_cost_startup, 8, es);
		ExplainPropertyFloat("nn_scan_qual_cost_per_tuple", NULL, plan->nn_scan_qual_cost_per_tuple, 8, es);
		ExplainPropertyInteger("nn_ngroup", NULL, plan->nn_ngroup, es);
		ExplainPropertyFloat("nn_group_startup_cost", NULL, plan->nn_group_startup_cost, 8, es);
		ExplainPropertyFloat("nn_group_run_cost", NULL, plan->nn_group_run_cost, 8, es);
		ExplainPropertyFloat("nn_group_input_run_cost", NULL, plan->nn_group_input_run_cost, 8, es);
		ExplainPropertyFloat("nn_nstream", NULL, plan->nn_nstream, 8, es);
		ExplainPropertyFloat("nn_material_nbytes", NULL, plan->nn_material_nbytes, 8, es);
		ExplainPropertyInteger("nn_aggstrategy", NULL, plan->nn_aggstrategy, es);
		ExplainPropertyInteger("nn_numGroups", NULL, plan->nn_numGroups, es);
		ExplainPropertyInteger("nn_output_tables", NULL, plan->nn_output_tables, es);
		ExplainPropertyInteger("nn_ngroups", NULL, plan->nn_ngroups, es);
		ExplainPropertyInteger("nn_ngroupcols", NULL, plan->nn_ngroupcols, es);
		ExplainPropertyInteger("nn_input_width", NULL, plan->nn_input_width, es);

		ExplainPropertyInteger("nn_ngroups_limit", NULL, plan->nn_ngroups_limit, es);
		ExplainPropertyInteger("nn_num_partitions", NULL, plan->nn_num_partitions, es);
		ExplainPropertyInteger("nn_depth", NULL, plan->nn_depth, es);
		ExplainPropertyFloat("nn_agg_transcost_startup", NULL, plan->nn_agg_transcost_startup, 8, es);
		ExplainPropertyFloat("nn_agg_transcost_per_tuple", NULL, plan->nn_agg_transcost_per_tuple, 8, es);
		ExplainPropertyFloat("nn_agg_finalcost_startup", NULL, plan->nn_agg_finalcost_startup, 8, es);
		ExplainPropertyFloat("nn_agg_finalcost_per_tuple", NULL, plan->nn_agg_finalcost_per_tuple, 8, es);
		ExplainPropertyFloat("nn_pages_written", NULL, plan->nn_pages_written, 5, es);
		ExplainPropertyFloat("nn_pages_read", NULL, plan->nn_pages_read, 5, es);
		ExplainPropertyFloat("nn_hashentrysize", NULL, plan->nn_hashentrysize, 8, es);
		ExplainPropertyFloat("nn_nbatches", NULL, plan->nn_nbatches, 5, es);
		ExplainPropertyInteger("nn_numpartcols", NULL, plan->nn_numpartcols, es);
		ExplainPropertyInteger("nn_numordercols", NULL, plan->nn_numordercols, es);
		ExplainPropertyInteger("nn_numgroupcols", NULL, plan->nn_numgroupcols, es);
		ExplainPropertyInteger("nn_pages_fetched", NULL, plan->nn_pages_fetched, es);
		ExplainPropertyFloat("nn_max_IO_cost", NULL, plan->nn_max_IO_cost, 8, es);
		ExplainPropertyFloat("nn_min_IO_cost", NULL, plan->nn_min_IO_cost, 8, es);
		ExplainPropertyFloat("nn_max_pages_fetched", NULL, plan->nn_max_pages_fetched, 8, es);
		ExplainPropertyFloat("nn_min_pages_fetched", NULL, plan->nn_min_pages_fetched, 8, es);

		ExplainPropertyFloat("nn_baserel_pages", NULL, plan->nn_baserel_pages, 8, es);
		ExplainPropertyFloat("nn_baserel_allvisfrac", NULL, plan->nn_baserel_allvisfrac, 8, es);
		ExplainPropertyBool("nn_index_unique", plan->nn_index_unique, es);
		ExplainPropertyInteger("nn_index_pages", NULL, plan->nn_index_pages, es);
		ExplainPropertyInteger("nn_index_tree_height", NULL, plan->nn_index_tree_height, es);
		ExplainPropertyInteger("nn_index_ncolumns", NULL, plan->nn_index_ncolumns, es);
		ExplainPropertyInteger("nn_index_nkeycolumns", NULL, plan->nn_index_nkeycolumns, es);
		ExplainPropertyFloat("nn_index_tuples", NULL, plan->nn_index_tuples, 8, es);
		ExplainPropertyFloat("nn_index_StartupCost", NULL, plan->nn_index_StartupCost, 8, es);
		ExplainPropertyFloat("nn_index_indexTotalCost", NULL, plan->nn_index_indexTotalCost, 8, es);
		ExplainPropertyFloat("nn_index_numIndexPages", NULL, plan->nn_index_numIndexPages, 8, es);
		ExplainPropertyFloat("nn_index_numIndexTuples", NULL, plan->nn_index_numIndexTuples, 8, es);
		ExplainPropertyFloat("nn_index_spc_random_page_cost", NULL, plan->nn_index_spc_random_page_cost, 5, es);
		ExplainPropertyFloat("nn_index_num_sa_scans", NULL, plan->nn_index_num_sa_scans, 8, es);

		/* for line wise equations */
		ExplainPropertyFloat("nn_eq_1", NULL, plan->nn_eq_1, 8, es);
		ExplainPropertyFloat("nn_eq_2", NULL, plan->nn_eq_2, 8, es);
		ExplainPropertyFloat("nn_eq_3", NULL, plan->nn_eq_3, 8, es);
		ExplainPropertyFloat("nn_eq_4", NULL, plan->nn_eq_4, 8, es);
		ExplainPropertyFloat("nn_eq_5", NULL, plan->nn_eq_5, 8, es);
		ExplainPropertyFloat("nn_eq_6", NULL, plan->nn_eq_6, 8, es);
		ExplainPropertyFloat("nn_eq_7", NULL, plan->nn_eq_7, 8, es);
		ExplainPropertyFloat("nn_eq_8", NULL, plan->nn_eq_8, 8, es);
		ExplainPropertyFloat("nn_eq_9", NULL, plan->nn_eq_9, 8, es);
		ExplainPropertyFloat("nn_eq_10", NULL, plan->nn_eq_10, 8, es);
		ExplainPropertyFloat("nn_eq_11", NULL, plan->nn_eq_11, 8, es);
		ExplainPropertyFloat("nn_eq_12", NULL, plan->nn_eq_12, 8, es);
		ExplainPropertyFloat("nn_eq_13", NULL, plan->nn_eq_13, 8, es);
		ExplainPropertyFloat("nn_eq_14", NULL, plan->nn_eq_14, 8, es);
	}
