...
{
	double s1 = path1->startup_cost;
	double s2 = path2->startup_cost;
	double t1 = path1->total_cost;
	double t2 = path2->total_cost;

	/* Added */
	if (use_cost_ml_server) {
		s1 = path1->ml_agg_startup_cost + ml_lambda_worst_case_minimization * path1->ml_agg_startup_std_cost;
		s2 = path2->ml_agg_startup_cost + ml_lambda_worst_case_minimization * path2->ml_agg_startup_std_cost;
		t1 = path1->ml_agg_total_cost + ml_lambda_worst_case_minimization * path1->ml_agg_total_std_cost;
		t2 = path2->ml_agg_total_cost + ml_lambda_worst_case_minimization * path2->ml_agg_total_std_cost;
	}
				   ...
				   double rows)
{
	...
	if (list_length(pathnode->subpaths) == 1)
	{
		...
		/* Added */
		pathnode->path.ml_agg_startup_cost += child->ml_agg_startup_cost;
		pathnode->path.ml_agg_total_cost += child->ml_agg_total_cost;
		pathnode->path.ml_agg_startup_std_cost += child->ml_agg_startup_std_cost;
		pathnode->path.ml_agg_total_std_cost += child->ml_agg_total_std_cost;
	}
	else
						 ...
						 Relids required_outer)
{
	...
	Cost		ml_agg_startup_cost = 0.;
	Cost		ml_agg_total_cost = 0.;
	Cost		ml_agg_startup_std_cost = 0.;
	Cost		ml_agg_total_std_cost = 0.;
	...
	foreach(l, subpaths)
	{
		...
		/* ML added */
		ml_agg_startup_cost += subpath->ml_agg_startup_cost;
		ml_agg_total_cost += subpath->ml_agg_total_cost;
		ml_agg_startup_std_cost += subpath->ml_agg_startup_std_cost;
		ml_agg_total_std_cost += subpath->ml_agg_total_std_cost;
		...
		else
		{
			...
			/* ML added */
			ml_agg_startup_cost += sort_path.ml_node_startup_cost;
			ml_agg_total_cost += sort_path.ml_node_total_cost;
			ml_agg_startup_std_cost += sort_path.ml_node_startup_std_cost;
			ml_agg_total_std_cost += sort_path.ml_node_total_std_cost;
		}
	...
	if (list_length(subpaths) == 1)
	{
		...
		/* ML Added */
		pathnode->path.ml_agg_startup_cost = ml_agg_startup_cost;
		pathnode->path.ml_agg_total_cost = ml_agg_total_cost;
		pathnode->path.ml_agg_startup_std_cost = ml_agg_startup_std_cost;
		pathnode->path.ml_agg_total_std_cost = ml_agg_total_std_cost;
	}
	else{
		cost_merge_append(&pathnode->path, root,
						  ...
						  pathnode->path.rows);
		/* ML Added */
		pathnode->path.ml_agg_startup_cost = ml_agg_startup_cost + pathnode->path.ml_node_startup_cost;
		pathnode->path.ml_agg_total_cost = ml_agg_total_cost + pathnode->path.ml_node_total_cost;
		pathnode->path.ml_agg_startup_std_cost = ml_agg_startup_std_cost + pathnode->path.ml_node_startup_std_cost;
		pathnode->path.ml_agg_total_std_cost = ml_agg_total_std_cost + pathnode->path.ml_node_total_std_cost;
	}
	return pathnode;
}
...
create_material_path(RelOptInfo *rel, Path *subpath)
{
	...
	/* ML Added */
	pathnode->path.ml_agg_startup_cost = subpath->ml_agg_startup_cost + pathnode->path.ml_node_startup_cost;
	pathnode->path.ml_agg_total_cost = subpath->ml_agg_total_cost + pathnode->path.ml_node_total_cost;
	pathnode->path.ml_agg_startup_std_cost = subpath->ml_agg_startup_std_cost + pathnode->path.ml_node_startup_std_cost;
	pathnode->path.ml_agg_total_std_cost = subpath->ml_agg_total_std_cost + pathnode->path.ml_node_total_std_cost;
					...
					bool singlerow, bool binary_mode, double calls)
{
	...
	/* ML Added 0. */
	pathnode->path.ml_agg_startup_cost = subpath->ml_agg_startup_cost;
	pathnode->path.ml_agg_total_cost = subpath->ml_agg_total_cost;
	pathnode->path.ml_agg_startup_std_cost = subpath->ml_agg_startup_std_cost;
	pathnode->path.ml_agg_total_std_cost = subpath->ml_agg_total_std_cost;
	return pathnode;
}
				   ...
				   SpecialJoinInfo *sjinfo)
{
	...
	/* assign a subpath value  */
	pathnode->path.ml_agg_startup_cost = subpath->ml_agg_total_cost;
	pathnode->path.ml_agg_total_cost = subpath->ml_agg_total_cost;
	pathnode->path.ml_agg_startup_std_cost = subpath->ml_agg_startup_std_cost;
	pathnode->path.ml_agg_total_std_cost = subpath->ml_agg_total_std_cost;
	...
	if (sjinfo->semi_can_btree && sjinfo->semi_can_hash)
	{
		if (use_cost_ml_server)
		{
			if (agg_path.ml_node_total_cost < sort_path.ml_node_total_cost)
	...
	if (pathnode->umethod == UNIQUE_PATH_HASH)
	{
		...
		/* ML Added */
		pathnode->path.ml_node_startup_cost = agg_path.ml_node_startup_cost;
		pathnode->path.ml_node_total_cost = agg_path.ml_node_total_cost;
		pathnode->path.ml_node_startup_std_cost = agg_path.ml_node_startup_std_cost;
		pathnode->path.ml_node_total_std_cost = agg_path.ml_node_total_std_cost;
	}
	else
	{
		...
		/* ML Added */
		pathnode->path.ml_node_startup_cost = sort_path.ml_node_startup_cost;
		pathnode->path.ml_node_total_cost = sort_path.ml_node_total_cost;
		pathnode->path.ml_node_startup_std_cost = sort_path.ml_node_startup_std_cost;
		pathnode->path.ml_node_total_std_cost = sort_path.ml_node_total_std_cost;
	}
	
	/* ML Added */
	pathnode->path.ml_agg_startup_cost += pathnode->path.ml_node_startup_cost;
	pathnode->path.ml_agg_total_cost += pathnode->path.ml_node_total_cost;
	pathnode->path.ml_agg_startup_std_cost += pathnode->path.ml_node_startup_std_cost;
	pathnode->path.ml_agg_total_std_cost += pathnode->path.ml_node_total_std_cost;
						 ...
						 Relids required_outer, double *rows)
{
	...
	else
	{
		...
		/* ML Added */
		ml_node_startup_cost += sort_path.ml_node_startup_cost;
		ml_node_total_cost += sort_path.ml_node_total_cost;
		ml_node_startup_std_cost += sort_path.ml_node_startup_std_cost;
		ml_node_total_std_cost += sort_path.ml_node_total_std_cost;
	}
	...
	/* Gather node added */
	ml_node_startup_cost += pathnode->path.ml_node_startup_cost;
	ml_node_total_cost += pathnode->path.ml_node_total_cost;
	ml_node_startup_std_cost += pathnode->path.ml_node_startup_std_cost;
	ml_node_total_std_cost += pathnode->path.ml_node_total_std_cost;

	pathnode->path.ml_node_startup_cost = ml_node_startup_cost;
	pathnode->path.ml_node_total_cost = ml_node_total_cost;
	pathnode->path.ml_node_startup_std_cost = ml_node_startup_std_cost;
	pathnode->path.ml_node_total_std_cost = ml_node_total_std_cost;
	
	pathnode->path.ml_agg_startup_cost = subpath->ml_agg_startup_cost + ml_node_startup_cost;
	pathnode->path.ml_agg_total_cost = subpath->ml_agg_total_cost + ml_node_total_cost;
	pathnode->path.ml_agg_startup_std_cost = subpath->ml_agg_startup_std_cost + ml_node_startup_std_cost;
	pathnode->path.ml_agg_total_std_cost = subpath->ml_agg_total_std_cost + ml_node_total_std_cost;
							 ...
							 double limit_tuples)
{
	...
	sort->spath.path.ml_agg_startup_cost = subpath->ml_agg_startup_cost + pathnode->path.ml_node_startup_cost;
	sort->spath.path.ml_agg_total_cost = subpath->ml_agg_total_cost + pathnode->path.ml_node_total_cost;
	sort->spath.path.ml_agg_startup_std_cost = subpath->ml_agg_startup_std_cost + pathnode->path.ml_node_startup_std_cost;
	sort->spath.path.ml_agg_total_std_cost = subpath->ml_agg_total_std_cost + pathnode->path.ml_node_total_std_cost;
	return sort;
}
				...
				double numGroups)
{
	...
	pathnode->path.ml_agg_startup_cost = subpath->ml_agg_total_cost + pathnode->path.ml_node_startup_cost;
	pathnode->path.ml_agg_total_cost = subpath->ml_agg_total_cost + pathnode->path.ml_node_total_cost;
	pathnode->path.ml_agg_startup_std_cost = subpath->ml_agg_startup_std_cost + pathnode->path.ml_node_startup_std_cost;
	pathnode->path.ml_agg_total_std_cost = subpath->ml_agg_total_std_cost + pathnode->path.ml_node_total_std_cost;
						 ...
						 double numGroups)
{
	...
	/* Added */
	Cost ml_node_startup_cost = 0.0;
	Cost ml_node_total_cost = 0.0;
	Cost ml_node_startup_std_cost = 0.0;
	Cost ml_node_total_std_cost = 0.0;
	...
	foreach(lc, rollups)
	{
		...
		else
		{
			...
			if (rollup->is_hashed || is_first_sort)
			{
				...
				ml_node_startup_cost += agg_path.ml_node_startup_cost;
				ml_node_total_cost += agg_path.ml_node_total_cost;
				ml_node_startup_std_cost += agg_path.ml_node_startup_std_cost;
				ml_node_total_std_cost += agg_path.ml_node_total_std_cost;
			}
			else
			{
	...
	pathnode->path.ml_agg_startup_cost += ml_node_startup_cost;
	pathnode->path.ml_agg_total_cost += ml_node_total_cost;
	pathnode->path.ml_agg_startup_std_cost += ml_node_startup_std_cost;
	pathnode->path.ml_agg_total_std_cost += ml_node_total_std_cost;
				  ...
				  int64 offset_est, int64 count_est)
{
	 ...
	 */
	double rows = pathnode->path.rows;
	...
	/* for ML estimation */
	Cost ml_agg_startup_cost = subpath->ml_agg_startup_cost;
	Cost ml_agg_total_cost = subpath->ml_agg_total_cost;
