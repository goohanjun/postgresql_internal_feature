...
copy_generic_path_info(Plan *dest, Path *src)
{
	...
	/* 
	 * Added by Hanjun.
	 * Statistics for learning neuralnetwork cost estimation model
 	 */
	dest->ml_node_startup_cost = src->ml_node_startup_cost;
	dest->ml_node_total_cost = src->ml_node_total_cost;
	dest->ml_node_startup_std_cost = src->ml_node_startup_std_cost;
	dest->ml_node_total_std_cost = src->ml_node_total_std_cost;
	
	dest->ml_agg_startup_cost = src->ml_agg_startup_cost;
	dest->ml_agg_total_cost = src->ml_agg_total_cost;
	dest->ml_agg_startup_std_cost = src->ml_agg_startup_std_cost;
	dest->ml_agg_total_std_cost = src->ml_agg_total_std_cost;

	dest->ml_inner_run_cost = src->ml_inner_run_cost;
	dest->ml_inner_rescan_run_cost = src->ml_inner_rescan_run_cost;
	dest->ml_inner_rescan_start_cost = src->ml_inner_rescan_start_cost;

	dest->nn_sort_left = src->nn_sort_left;
	dest->ml_sort_left_cost = src->ml_sort_left_cost;
	dest->ml_sort_left_std_cost = src->ml_sort_left_std_cost;

	dest->nn_sort_right = src->nn_sort_right;
	dest->ml_sort_right_cost = src->ml_sort_right_cost;
	dest->ml_sort_right_std_cost = src->ml_sort_right_std_cost;

	// for nestloop
	dest->nn_restrict_qual_cost_startup = src->nn_restrict_qual_cost_startup;
	dest->nn_pathtarget_cost_startup = src->nn_pathtarget_cost_startup;
	dest->nn_pathtarget_cost_per_tuple = src->nn_pathtarget_cost_per_tuple;

	dest->nn_outer_path_rows = src->nn_outer_path_rows;
	dest->nn_inner_path_rows = src->nn_inner_path_rows;
	dest->nn_path_rows = src->nn_path_rows;
	dest->nn_per_tuple = src->nn_per_tuple;

	dest->nn_outer_matched_rows = src->nn_outer_matched_rows;
	dest->nn_outer_unmatched_rows = src->nn_outer_unmatched_rows;
	dest->nn_inner_scan_frac = src->nn_inner_scan_frac;

	dest->nn_ntuples = src->nn_ntuples;
	dest->nn_parallel_worker = src->nn_parallel_worker;
	dest->nn_jointype = src->nn_jointype;
	dest->nn_match_count = src->nn_match_count;
	dest->nn_inner_unique = src->nn_inner_unique;
	dest->nn_has_indexed_join_quals = src->nn_has_indexed_join_quals;

	// for merge
	dest->nn_outer_rows = src->nn_outer_rows;
	dest->nn_inner_rows = src->nn_inner_rows;
	dest->nn_outer_skip_rows = src->nn_outer_skip_rows;
	dest->nn_inner_skip_rows = src->nn_inner_skip_rows;
	
	dest->nn_rescanratio = src->nn_rescanratio;
	dest->nn_outerstartsel = src->nn_outerstartsel;
	dest->nn_outerendsel = src->nn_outerendsel;
	dest->nn_innerstartsel = src->nn_innerstartsel;
	dest->nn_innerendsel = src->nn_innerendsel;

	dest->nn_merge_qual_cost_startup = src->nn_merge_qual_cost_startup;
	dest->nn_merge_qual_cost_per_tuple = src->nn_merge_qual_cost_per_tuple;
	dest->nn_qp_qual_cost_startup = src->nn_qp_qual_cost_startup;
	dest->nn_qp_qual_cost_per_tuple = src->nn_qp_qual_cost_per_tuple;
	
	dest->nn_mergejointuples = src->nn_mergejointuples;
	dest->nn_rescannedtuples = src->nn_rescannedtuples;
	
	dest->nn_materialize_inner = src->nn_materialize_inner;
	dest->nn_skip_mark_restore = src->nn_skip_mark_restore;

	// for hash
	dest->nn_numbuckets = src->nn_numbuckets;
	dest->nn_numbatches = src->nn_numbatches;
	dest->nn_num_hashclauses = src->nn_num_hashclauses;
	dest->nn_outerpages = src->nn_outerpages;
	dest->nn_innerpages = src->nn_innerpages;
	dest->nn_inner_path_rows_total = src->nn_inner_path_rows_total;
	dest->nn_hashjointuples = src->nn_hashjointuples;
	dest->nn_virtualbuckets = src->nn_virtualbuckets;
	dest->nn_innerbucketsize = src->nn_innerbucketsize;
	dest->nn_innermcvfreq = src->nn_innermcvfreq;
	dest->nn_hash_qual_cost_startup = src->nn_hash_qual_cost_startup;
	dest->nn_hash_qual_cost_per_tuple = src->nn_hash_qual_cost_per_tuple;

	dest->nn_spc_seq_page_cost = src->nn_spc_seq_page_cost;
	dest->nn_pages = src->nn_pages;
	dest->nn_spc_random_page_cost = src->nn_spc_random_page_cost;
	dest->nn_comparison_cost = src->nn_comparison_cost;
	dest->nn_parallel_tuple_cost = src->nn_parallel_tuple_cost;
	dest->nn_num_workers = src->nn_num_workers;

	dest->nn_indexStartupCost = src->nn_indexStartupCost;
	dest->nn_indexTotalCost = src->nn_indexTotalCost;
	dest->nn_indexselectivity = src->nn_indexselectivity;
	dest->nn_indexcorrelation = src->nn_indexcorrelation;
	dest->nn_indexpage = src->nn_indexpage;
	dest->nn_max_pages_fetched = src->nn_max_pages_fetched;
	dest->nn_min_pages_fetched = src->nn_min_pages_fetched;
	dest->nn_max_IO_cost = src->nn_max_IO_cost;
	dest->nn_min_IO_cost = src->nn_min_IO_cost;
	dest->nn_loopcount = src->nn_loopcount;
	dest->nn_indexonly = src->nn_indexonly;
	dest->nn_partial_path = src->nn_partial_path;

    dest->nn_pages_fetched = src->nn_pages_fetched;
	dest->nn_selectivity = src->nn_selectivity;
	dest->nn_iscurrentof = src->nn_iscurrentof;
	dest->nn_scan_qual_cost_startup = src->nn_scan_qual_cost_startup;
	dest->nn_scan_qual_cost_per_tuple = src->nn_scan_qual_cost_per_tuple;

	dest->nn_ngroup = src->nn_ngroup;
	dest->nn_group_startup_cost = src->nn_group_startup_cost;
	dest->nn_group_run_cost = src->nn_group_run_cost;
	dest->nn_group_input_run_cost = src->nn_group_input_run_cost;
	
	dest->nn_nstream = src->nn_nstream;
	dest->nn_material_nbytes = src->nn_material_nbytes;

	dest->nn_aggstrategy = src->nn_aggstrategy;
	dest->nn_numGroups = src->nn_numGroups;
	dest->nn_output_tables = src->nn_output_tables;
	dest->nn_ngroups = src->nn_ngroups;
	dest->nn_ngroupcols = src->nn_ngroupcols;
	dest->nn_input_width = src->nn_input_width;
	dest->nn_ngroups_limit = src->nn_ngroups_limit;
	dest->nn_num_partitions = src->nn_num_partitions;
	dest->nn_depth = src->nn_depth;

	dest->nn_agg_transcost_startup = src->nn_agg_transcost_startup;
	dest->nn_agg_transcost_per_tuple = src->nn_agg_transcost_per_tuple;
	dest->nn_agg_finalcost_startup = src->nn_agg_finalcost_startup;
	dest->nn_agg_finalcost_per_tuple = src->nn_agg_finalcost_per_tuple;
	dest->nn_pages_written = src->nn_pages_written;
	dest->nn_pages_read = src->nn_pages_read;
	dest->nn_hashentrysize = src->nn_hashentrysize;
	dest->nn_nbatches = src->nn_nbatches;

	dest->nn_numpartcols = src->nn_numpartcols;
	dest->nn_numordercols = src->nn_numordercols;
	dest->nn_numgroupcols = src->nn_numgroupcols;

	/* index */
	dest->nn_baserel_pages = src->nn_baserel_pages;
	dest->nn_baserel_allvisfrac = src->nn_baserel_allvisfrac;
	dest->nn_index_pages = src->nn_index_pages;
	dest->nn_index_tuples = src->nn_index_tuples;
	dest->nn_index_tree_height = src->nn_index_tree_height;
	dest->nn_index_ncolumns = src->nn_index_ncolumns;
	dest->nn_index_nkeycolumns = src->nn_index_nkeycolumns;
	dest->nn_index_unique = src->nn_index_unique;
	dest->nn_index_StartupCost = src->nn_index_StartupCost;
	dest->nn_index_indexTotalCost = src->nn_index_indexTotalCost;
	dest->nn_index_numIndexPages = src->nn_index_numIndexPages;
	dest->nn_index_numIndexTuples = src->nn_index_numIndexTuples;
	dest->nn_index_spc_random_page_cost = src->nn_index_spc_random_page_cost;
	dest->nn_index_num_sa_scans = src->nn_index_num_sa_scans;

	/* line-wise equations */
	dest->nn_eq_1 = src->nn_eq_1;
	dest->nn_eq_2 = src->nn_eq_2;
	dest->nn_eq_3 = src->nn_eq_3;
	dest->nn_eq_4 = src->nn_eq_4;
	dest->nn_eq_5 = src->nn_eq_5;
	dest->nn_eq_6 = src->nn_eq_6;
	dest->nn_eq_7 = src->nn_eq_7;
	dest->nn_eq_8 = src->nn_eq_8;
	dest->nn_eq_9 = src->nn_eq_9;
	dest->nn_eq_10 = src->nn_eq_10;
	dest->nn_eq_11 = src->nn_eq_11;
	dest->nn_eq_12 = src->nn_eq_12;
	dest->nn_eq_13 = src->nn_eq_13;
	dest->nn_eq_14 = src->nn_eq_14;
}
...
materialize_finished_plan(Plan *subplan)
{
	...
	/* added for ML */
	matplan->ml_agg_startup_cost = subplan->ml_agg_startup_cost + matpath.ml_node_startup_cost;
	matplan->ml_agg_total_cost = subplan->ml_agg_total_cost + matpath.ml_node_total_cost;
	matplan->ml_agg_startup_std_cost = subplan->ml_agg_startup_std_cost + matpath.ml_node_startup_std_cost;
	matplan->ml_agg_total_std_cost = subplan->ml_agg_total_std_cost + matpath.ml_node_total_std_cost;
	return matplan;
}
