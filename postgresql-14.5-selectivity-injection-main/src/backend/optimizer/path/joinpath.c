...
extern void print_path(PlannerInfo *root, Path *path, int indent);
				  ...
				  JoinPathExtraData *extra)
{
	...
	if (print_plan_opt_path){
		printf("\ntry_nestloop_path() starts\n");
		printf("cost startup: %f, total: %f\n", workspace.startup_cost, workspace.total_cost);
		debug_print_rel(root, joinrel);
		printf("outer_path\n");
		print_path(root, outer_path, 8);
		printf("inner_path\n");
		print_path(root, inner_path, 8);
	}
