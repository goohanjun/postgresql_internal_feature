...
bool        print_plan_opt_path = false;
double      ml_lambda_worst_case_minimization = 0.;
bool        use_cost_ml_server = false;
bool        use_join_ml_only = false;
char		*addr_cost_ml_server = NULL;
...
// Write callback function to accumulate response data
struct libcurl_memory {
    char *response;
    size_t size;
};
...
static size_t write_callback(void *data, size_t size, size_t nmemb, void *userp) {
    ...
    struct libcurl_memory *mem = (struct libcurl_memory *)userp;

    if (real_size == 0) {
        return 0; 
    }

    char *ptr = realloc(mem->response, mem->size + real_size + 1); // 1은 null terminator를 위한 공간
    if (ptr == NULL) {
    ...
    mem->size += real_size;
    mem->response[mem->size] = 0; // 문자열 종료를 위해 null 추가
    return real_size;
}

void 
inference_ml_cost(char* node_type, Path *path, double *startup_cost, double *total_cost, double *startup_std_cost, double *total_std_cost)
{
	if (!use_cost_ml_server){
		return;
	}
	if (path == NULL) {
 	   elog(ERROR, "Path is NULL");
 	   return;
	}
	/*
	elog(INFO, "startup_cost: %f", path->startup_cost);
	elog(INFO, "total_cost: %f", path->total_cost);
	*/

	/* generate JSON for the path using cJSON */
	cJSON *json_obj = cJSON_CreateObject();
	if (json_obj == NULL) {
 		elog(ERROR, "Failed to create JSON object");
 		return;
	}

	cJSON_AddStringToObject(json_obj, "Node-Type", node_type);
	cJSON_AddNumberToObject(json_obj, "Startup-Cost", *startup_cost);
	cJSON_AddNumberToObject(json_obj, "Total-Cost", *total_cost);
	
	cJSON_AddNumberToObject(json_obj, "nn_sort_left", path->nn_sort_left);
	cJSON_AddNumberToObject(json_obj, "nn_sort_right", path->nn_sort_right);

	cJSON_AddNumberToObject(json_obj, "nn_restrict_qual_cost_startup", path->nn_restrict_qual_cost_startup);
	cJSON_AddNumberToObject(json_obj, "nn_pathtarget_cost_startup", path->nn_pathtarget_cost_startup);
	cJSON_AddNumberToObject(json_obj, "nn_pathtarget_cost_per_tuple", path->nn_pathtarget_cost_per_tuple);

	cJSON_AddNumberToObject(json_obj, "nn_outer_path_rows", path->nn_outer_path_rows);
	cJSON_AddNumberToObject(json_obj, "nn_inner_path_rows", path->nn_inner_path_rows);
	cJSON_AddNumberToObject(json_obj, "nn_path_rows", path->nn_path_rows);
	cJSON_AddNumberToObject(json_obj, "nn_per_tuple", path->nn_per_tuple);
	cJSON_AddNumberToObject(json_obj, "nn_outer_matched_rows", path->nn_outer_matched_rows);
	cJSON_AddNumberToObject(json_obj, "nn_outer_unmatched_rows", path->nn_outer_unmatched_rows);
	cJSON_AddNumberToObject(json_obj, "nn_inner_scan_frac", path->nn_inner_scan_frac);
	cJSON_AddNumberToObject(json_obj, "nn_ntuples", path->nn_ntuples);
	cJSON_AddNumberToObject(json_obj, "nn_parallel_worker", path->nn_parallel_worker);
	cJSON_AddNumberToObject(json_obj, "nn_jointype", path->nn_jointype);
	cJSON_AddNumberToObject(json_obj, "nn_match_count", path->nn_match_count);
	cJSON_AddBoolToObject(json_obj, "nn_inner_unique", path->nn_inner_unique);
	cJSON_AddBoolToObject(json_obj, "nn_has_indexed_join_quals", path->nn_has_indexed_join_quals);
	cJSON_AddNumberToObject(json_obj, "nn_outer_rows", path->nn_outer_rows);
	cJSON_AddNumberToObject(json_obj, "nn_inner_rows", path->nn_inner_rows);
	cJSON_AddNumberToObject(json_obj, "nn_outer_skip_rows", path->nn_outer_skip_rows);
	cJSON_AddNumberToObject(json_obj, "nn_inner_skip_rows", path->nn_inner_skip_rows);
	cJSON_AddNumberToObject(json_obj, "nn_rescanratio", path->nn_rescanratio);
	cJSON_AddNumberToObject(json_obj, "nn_outerstartsel", path->nn_outerstartsel);
	cJSON_AddNumberToObject(json_obj, "nn_outerendsel", path->nn_outerendsel);
	cJSON_AddNumberToObject(json_obj, "nn_innerstartsel", path->nn_innerstartsel);
	cJSON_AddNumberToObject(json_obj, "nn_innerendsel", path->nn_innerendsel);
	cJSON_AddNumberToObject(json_obj, "nn_merge_qual_cost_startup", path->nn_merge_qual_cost_startup);
	cJSON_AddNumberToObject(json_obj, "nn_merge_qual_cost_per_tuple", path->nn_merge_qual_cost_per_tuple);
	cJSON_AddNumberToObject(json_obj, "nn_qp_qual_cost_startup", path->nn_qp_qual_cost_startup);
	cJSON_AddNumberToObject(json_obj, "nn_qp_qual_cost_per_tuple", path->nn_qp_qual_cost_per_tuple);
	cJSON_AddNumberToObject(json_obj, "nn_mergejointuples", path->nn_mergejointuples);
	cJSON_AddNumberToObject(json_obj, "nn_rescannedtuples", path->nn_rescannedtuples);
	cJSON_AddBoolToObject(json_obj, "nn_materialize_inner", path->nn_materialize_inner);
	cJSON_AddBoolToObject(json_obj, "nn_skip_mark_restore", path->nn_skip_mark_restore);
	cJSON_AddNumberToObject(json_obj, "nn_numbuckets", path->nn_numbuckets);
	cJSON_AddNumberToObject(json_obj, "nn_numbatches", path->nn_numbatches);
	cJSON_AddNumberToObject(json_obj, "nn_num_hashclauses", path->nn_num_hashclauses);
	cJSON_AddNumberToObject(json_obj, "nn_innerpages", path->nn_innerpages);
	cJSON_AddNumberToObject(json_obj, "nn_outerpages", path->nn_outerpages);
	cJSON_AddNumberToObject(json_obj, "nn_inner_path_rows_total", path->nn_inner_path_rows_total);
	cJSON_AddNumberToObject(json_obj, "nn_hashjointuples", path->nn_hashjointuples);
	cJSON_AddNumberToObject(json_obj, "nn_virtualbuckets", path->nn_virtualbuckets);
	cJSON_AddNumberToObject(json_obj, "nn_innerbucketsize", path->nn_innerbucketsize);
	cJSON_AddNumberToObject(json_obj, "nn_innermcvfreq", path->nn_innermcvfreq);
	cJSON_AddNumberToObject(json_obj, "nn_hash_qual_cost_startup", path->nn_hash_qual_cost_startup);
	cJSON_AddNumberToObject(json_obj, "nn_hash_qual_cost_per_tuple", path->nn_hash_qual_cost_per_tuple);
	cJSON_AddNumberToObject(json_obj, "nn_spc_seq_page_cost", path->nn_spc_seq_page_cost);
	cJSON_AddNumberToObject(json_obj, "nn_pages", path->nn_pages);
	cJSON_AddNumberToObject(json_obj, "nn_spc_random_page_cost", path->nn_spc_random_page_cost);
	cJSON_AddNumberToObject(json_obj, "nn_comparison_cost", path->nn_comparison_cost);
	cJSON_AddNumberToObject(json_obj, "nn_parallel_tuple_cost", path->nn_parallel_tuple_cost);
	cJSON_AddNumberToObject(json_obj, "nn_num_workers", path->nn_num_workers);
	cJSON_AddNumberToObject(json_obj, "nn_indexStartupCost", path->nn_indexStartupCost);
	cJSON_AddNumberToObject(json_obj, "nn_indexTotalCost", path->nn_indexTotalCost);
	cJSON_AddNumberToObject(json_obj, "nn_indexselectivity", path->nn_indexselectivity);
	cJSON_AddNumberToObject(json_obj, "nn_indexcorrelation", path->nn_indexcorrelation);
	cJSON_AddNumberToObject(json_obj, "nn_indexpage", path->nn_indexpage);
	cJSON_AddNumberToObject(json_obj, "nn_max_pages_fetched", path->nn_max_pages_fetched);
	cJSON_AddNumberToObject(json_obj, "nn_min_pages_fetched", path->nn_min_pages_fetched);
	cJSON_AddNumberToObject(json_obj, "nn_max_IO_cost", path->nn_max_IO_cost);
	cJSON_AddNumberToObject(json_obj, "nn_min_IO_cost", path->nn_min_IO_cost);
	cJSON_AddNumberToObject(json_obj, "nn_loopcount", path->nn_loopcount);
	cJSON_AddBoolToObject(json_obj, "nn_indexonly", path->nn_indexonly);
	cJSON_AddBoolToObject(json_obj, "nn_partial_path", path->nn_partial_path);
	cJSON_AddNumberToObject(json_obj, "nn_pages_fetched", path->nn_pages_fetched);
	cJSON_AddNumberToObject(json_obj, "nn_selectivity", path->nn_selectivity);
	cJSON_AddBoolToObject(json_obj, "nn_iscurrentof", path->nn_iscurrentof);
	cJSON_AddNumberToObject(json_obj, "nn_scan_qual_cost_startup", path->nn_scan_qual_cost_startup);
	cJSON_AddNumberToObject(json_obj, "nn_scan_qual_cost_per_tuple", path->nn_scan_qual_cost_per_tuple);
	cJSON_AddNumberToObject(json_obj, "nn_ngroup", path->nn_ngroup);
	cJSON_AddNumberToObject(json_obj, "nn_group_startup_cost", path->nn_group_startup_cost);
	cJSON_AddNumberToObject(json_obj, "nn_group_run_cost", path->nn_group_run_cost);
	cJSON_AddNumberToObject(json_obj, "nn_group_input_run_cost", path->nn_group_input_run_cost);
	cJSON_AddNumberToObject(json_obj, "nn_nstream", path->nn_nstream);
	cJSON_AddNumberToObject(json_obj, "nn_material_nbytes", path->nn_material_nbytes);
	cJSON_AddNumberToObject(json_obj, "nn_aggstrategy", path->nn_aggstrategy);
	cJSON_AddNumberToObject(json_obj, "nn_numGroups", path->nn_numGroups);
	cJSON_AddNumberToObject(json_obj, "nn_output_tables", path->nn_output_tables);
	cJSON_AddNumberToObject(json_obj, "nn_ngroups", path->nn_ngroups);
	cJSON_AddNumberToObject(json_obj, "nn_ngroupcols", path->nn_ngroupcols);
	cJSON_AddNumberToObject(json_obj, "nn_input_width", path->nn_input_width);
	cJSON_AddNumberToObject(json_obj, "nn_ngroups_limit", path->nn_ngroups_limit);
	cJSON_AddNumberToObject(json_obj, "nn_num_partitions", path->nn_num_partitions);
	cJSON_AddNumberToObject(json_obj, "nn_depth", path->nn_depth);
	cJSON_AddNumberToObject(json_obj, "nn_agg_transcost_startup", path->nn_agg_transcost_startup);
    cJSON_AddNumberToObject(json_obj, "nn_agg_transcost_per_tuple", path->nn_agg_transcost_per_tuple);
    cJSON_AddNumberToObject(json_obj, "nn_agg_finalcost_startup", path->nn_agg_finalcost_startup);
	cJSON_AddNumberToObject(json_obj, "nn_agg_finalcost_per_tuple", path->nn_agg_finalcost_per_tuple);
	cJSON_AddNumberToObject(json_obj, "nn_pages_written", path->nn_pages_written);
	cJSON_AddNumberToObject(json_obj, "nn_pages_read", path->nn_pages_read);
	cJSON_AddNumberToObject(json_obj, "nn_hashentrysize", path->nn_hashentrysize);
	cJSON_AddNumberToObject(json_obj, "nn_nbatches", path->nn_nbatches);
	cJSON_AddNumberToObject(json_obj, "nn_numpartcols", path->nn_numpartcols);
	cJSON_AddNumberToObject(json_obj, "nn_numordercols", path->nn_numordercols);
	cJSON_AddNumberToObject(json_obj, "nn_numgroupcols", path->nn_numgroupcols);

	/* Index */
	cJSON_AddNumberToObject(json_obj, "nn_baserel_pages", path->nn_baserel_pages);
	cJSON_AddNumberToObject(json_obj, "nn_baserel_allvisfrac", path->nn_baserel_allvisfrac);
	cJSON_AddNumberToObject(json_obj, "nn_index_pages", path->nn_index_pages);
	cJSON_AddNumberToObject(json_obj, "nn_index_StartupCost", path->nn_index_StartupCost);
	cJSON_AddNumberToObject(json_obj, "nn_index_indexTotalCost", path->nn_index_indexTotalCost);
	cJSON_AddNumberToObject(json_obj, "nn_index_tuples", path->nn_index_tuples);
	cJSON_AddNumberToObject(json_obj, "nn_index_tree_height", path->nn_index_tree_height);
	cJSON_AddNumberToObject(json_obj, "nn_index_ncolumns", path->nn_index_ncolumns);
	cJSON_AddNumberToObject(json_obj, "nn_index_nkeycolumns", path->nn_index_nkeycolumns);
	cJSON_AddNumberToObject(json_obj, "nn_index_unique", path->nn_index_unique);
	cJSON_AddNumberToObject(json_obj, "nn_index_spc_random_page_cost", path->nn_index_spc_random_page_cost);
	cJSON_AddNumberToObject(json_obj, "nn_index_numIndexPages", path->nn_index_numIndexPages);
	cJSON_AddNumberToObject(json_obj, "nn_index_numIndexTuples", path->nn_index_numIndexTuples);
	cJSON_AddNumberToObject(json_obj, "nn_index_num_sa_scans", path->nn_index_num_sa_scans);

	/* equations */
	cJSON_AddNumberToObject(json_obj, "nn_eq_1", path->nn_eq_1);
	cJSON_AddNumberToObject(json_obj, "nn_eq_2", path->nn_eq_2);
	cJSON_AddNumberToObject(json_obj, "nn_eq_3", path->nn_eq_3);
	cJSON_AddNumberToObject(json_obj, "nn_eq_4", path->nn_eq_4);
	cJSON_AddNumberToObject(json_obj, "nn_eq_5", path->nn_eq_5);
	cJSON_AddNumberToObject(json_obj, "nn_eq_6", path->nn_eq_6);
	cJSON_AddNumberToObject(json_obj, "nn_eq_7", path->nn_eq_7);
	cJSON_AddNumberToObject(json_obj, "nn_eq_8", path->nn_eq_8);
	cJSON_AddNumberToObject(json_obj, "nn_eq_9", path->nn_eq_9);
	cJSON_AddNumberToObject(json_obj, "nn_eq_10", path->nn_eq_10);
	cJSON_AddNumberToObject(json_obj, "nn_eq_11", path->nn_eq_11);
	cJSON_AddNumberToObject(json_obj, "nn_eq_12", path->nn_eq_12);
	cJSON_AddNumberToObject(json_obj, "nn_eq_13", path->nn_eq_13);
	cJSON_AddNumberToObject(json_obj, "nn_eq_14", path->nn_eq_14);
	
	char *json_string = cJSON_Print(json_obj);
	cJSON_Delete(json_obj);

	if (json_string == NULL) {
		elog(ERROR, "Failed to print JSON object");
		cJSON_Delete(json_obj);
  		return;
	}
	...
	if (!curl) {
		elog(INFO, "Failed to initialize CURL");
		free(json_string);
		cJSON_Delete(json_obj);
		return;
	}
	...
	/* grown as needed by the realloc above */
	struct libcurl_memory req;
	req.response = malloc(1);  
	req.size = 0;
	...
	if(curl) {
		...
		curl_easy_setopt(curl, CURLOPT_URL, "http://172.17.0.1:8000/test");
		...
		/* Parse response of POST */
		cJSON *json_response = cJSON_Parse(req.response);
		free(req.response);

		cJSON *startup_cost_str = cJSON_GetObjectItemCaseSensitive(json_response, "startup_cost");

		if (cJSON_IsNumber(startup_cost_str)) {
			*startup_cost = startup_cost_str->valuedouble;
		}

		cJSON *total_cost_str = cJSON_GetObjectItemCaseSensitive(json_response, "total_cost");
		if (cJSON_IsNumber(total_cost_str)) {
			*total_cost = total_cost_str->valuedouble;
		}

		cJSON *startup_std_str = cJSON_GetObjectItemCaseSensitive(json_response, "startup_std_cost");
		if (cJSON_IsNumber(startup_std_str)) {
			*startup_std_cost = startup_std_str->valuedouble;
		}

		cJSON *total_std_str = cJSON_GetObjectItemCaseSensitive(json_response, "total_std_cost");
		if (cJSON_IsNumber(total_std_str)) {
			*total_std_cost = total_std_str->valuedouble;
		}
...
standard_join_search(PlannerInfo *root, int levels_needed, List *initial_rels)
{
	...
	for (lev = 2; lev <= levels_needed; lev++)
	{
		...
		foreach(lc, root->join_rel_level[lev])
		{
			...
			if (print_plan_opt_path)
				printf("\nafter generate_partitionwise_join_paths()\n");
				debug_print_rel(root, rel);
#ifdef OPTIMIZER_DEBUG
#endif
		}
