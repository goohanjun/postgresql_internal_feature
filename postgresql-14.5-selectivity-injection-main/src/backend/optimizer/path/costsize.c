...
extern bool print_plan_opt_path;
extern bool use_cost_ml_server;
extern double ml_lambda_worst_case_minimization;
extern bool use_join_ml_only;
extern char *addr_cost_ml_server;
...
static void cost_rescan_ml(PlannerInfo *root, Path *path, Cost *rescan_startup_cost, Cost *rescan_total_cost);
...
static void
print_relids_fp(FILE *fp, PlannerInfo *root, Relids relids)
{
	int x;
	bool first = true;
...
{
	// elog(WARNING, "fprint_const for %s", str);

	if (type_category == TYPCATEGORY_ARRAY)
	{
		// elog(WARNING, "TYPCATEGORY_ARRAY");
		...
		// elog(WARNING, "splitArrayString - start");
		char **element_strings = splitArrayString(str, &size);
		// elog(WARNING, "splitArrayString - end");

		// printf("element_strings address = %p\n", (void*)element_strings);
		// for (int i = 0; i < size; i++) {
		// 	printf("element_strings[%d] address = %p, value = %s\n", i, (void*)element_strings[i], element_strings[i]);
		// }
		...
		for (int i = 0; i < size; i++)
		{
			...
			// elog(WARNING, "fprint_const recursive with %d - start", i);
			fprint_const(fp, element_type_oid, element_type_category, element_strings[i]);
			// elog(WARNING, "fprint_const recursive with %d - end", i);
	...
	{
		// elog(WARNING, "TYPCATEGORY_STRING");
	...
	// elog(WARNING, "END of fprint_const");
}
...
fprint_expr(FILE *fp, const Node *expr, const List *rtable)
{
	...
	// elog(WARNING, "fprint_expr type: %d", expr->type);
	if (expr == NULL)
	{
		// elog(WARNING, "expr = NULL");
		fprintf(fp, "<>");
		return;
	}
	...
	else if (IsA(expr, Const))
	{
		...
		if (c->constisnull)
		{
			// elog(WARNING, "const is null");
			fprintf(fp, "NULL");
			return;
		}
		// elog(WARNING, "getTypeOutputInfo");
		// elog(WARNING, "c->consttype %d", c->consttype);
	...
	{
		// elog(WARNING, "expr = ScalarArrayOpExpr");
	...
	else if (IsA(expr, BoolExpr))
	{
		...
		if (e->args == NULL)
		{
			// elog(WARNING, "e->args == NULL");
			return;
		}

		// elog(WARNING, "Debugging: e->boolop=%d", e->boolop);

		if (e->boolop == NOT_EXPR)
		{
			// elog(WARNING, "Debugging: e->boolop NOT_EXPR");
			fprintf(fp, "NOT (");
			if (linitial(e->args)) // NULL 체크 추가
			{
				fprint_expr(fp, linitial(e->args), rtable);
			}
			fprintf(fp, ")");
		...
		{
			// elog(WARNING, "Debugging: e->boolop else");
			char *op_name;
...
print_basic_rel(FILE *fp, PlannerInfo *root, RelOptInfo *rel)
{
	...
	print_relids_fp(fp, root, rel->relids);
	fprintf(fp, "): rows=%.0f width=%d\n", rel->rows, rel->reltarget->width);

	if (rel->baserestrictinfo)
	{
		fprintf(fp, "\tbaserestrictinfo: ");
		print_restrictclauses_fp(fp, root, rel->baserestrictinfo);
		fprintf(fp, "\n");
	}
}
				  ...
				  RelOptInfo *baserel, ParamPathInfo *param_info)
{
	...
	double spc_seq_page_cost = 0.;
	Cost cpu_per_tuple = 0.;
	/* Initialize */
	QualCost qpqual_cost;
	qpqual_cost.per_tuple = 0.;
	qpqual_cost.startup = 0.;
	...
	/* Added by hanjun. */
	/* equations */
	path->nn_eq_1 = qpqual_cost.startup;
	path->nn_eq_2 = cpu_per_tuple * baserel->tuples;
	path->nn_eq_3 = path->pathtarget->cost.startup;
	path->nn_eq_4 = path->pathtarget->cost.per_tuple * path->rows;
	path->nn_eq_5 = disk_run_cost;

	path->nn_input_width = path->pathtarget->width;
	path->nn_path_rows = path->rows;
	path->nn_spc_seq_page_cost = spc_seq_page_cost;
	path->nn_pages = baserel->pages;
	path->nn_ntuples = baserel->tuples;
	path->nn_parallel_worker = path->parallel_workers;
	path->nn_qp_qual_cost_startup = qpqual_cost.startup;
	path->nn_qp_qual_cost_per_tuple = qpqual_cost.per_tuple;

	path->nn_pathtarget_cost_startup = path->pathtarget->cost.startup;
	path->nn_pathtarget_cost_per_tuple = path->pathtarget->cost.per_tuple;

	path->startup_cost = startup_cost;
	path->total_cost = startup_cost + cpu_run_cost + disk_run_cost;

	Cost ml_node_startup_cost = 0.;
	Cost ml_node_total_cost = 0.;
	Cost ml_node_startup_std_cost = 0.;
	Cost ml_node_total_std_cost = 0.;

	inference_ml_cost("Seq Scan", path, &ml_node_startup_cost, &ml_node_total_cost, &ml_node_startup_std_cost, &ml_node_total_std_cost);
	path->ml_node_startup_cost = ml_node_startup_cost;
	path->ml_node_total_cost = ml_node_total_cost;
	path->ml_node_startup_std_cost = ml_node_startup_std_cost;
	path->ml_node_total_std_cost = ml_node_total_std_cost;
	
	/* Base Relation */
	path->ml_agg_startup_cost = ml_node_startup_cost;
	path->ml_agg_total_cost = ml_node_total_cost;
	path->ml_agg_startup_std_cost = ml_node_startup_std_cost;
	path->ml_agg_total_std_cost = ml_node_total_std_cost;
}
 ...
 * 'param_info' is the ParamPathInfo if this is a parameterized path, else NULL
 */
void cost_samplescan(Path *path, PlannerInfo *root,
					 RelOptInfo *baserel, ParamPathInfo *param_info)
{
	Cost startup_cost = 0;
	Cost run_cost = 0;
	...
	/* equations */
	path->nn_eq_1 = qpqual_cost.startup;
	path->nn_eq_2 = path->pathtarget->cost.startup;
	path->nn_eq_3 = spc_page_cost * baserel->pages;
	path->nn_eq_4 = cpu_per_tuple * baserel->tuples;
	path->nn_eq_5 = path->pathtarget->cost.per_tuple * path->rows;

	path->nn_input_width = path->pathtarget->width;
	path->nn_path_rows = path->rows;
	path->nn_spc_seq_page_cost = spc_seq_page_cost;
	path->nn_spc_random_page_cost = spc_random_page_cost;
	path->nn_pages = baserel->pages;
	path->nn_ntuples = baserel->tuples;
	path->nn_qp_qual_cost_startup = qpqual_cost.startup;
	path->nn_qp_qual_cost_per_tuple = qpqual_cost.per_tuple;
	...
	inference_ml_cost("Sample Scan", path, &ml_node_startup_cost, &ml_node_total_cost, &ml_node_startup_std_cost, &ml_node_total_std_cost);
	path->ml_node_startup_cost = ml_node_startup_cost;
	path->ml_node_total_cost = ml_node_total_cost;
	path->ml_node_startup_std_cost = ml_node_startup_std_cost;
	path->ml_node_total_std_cost = ml_node_total_std_cost;
				 ...
				 double *rows)
{
	...
	/* ML equations */
	path->path.nn_eq_1 = path->subpath->startup_cost;
	path->path.nn_eq_2 = parallel_setup_cost;
	path->path.nn_eq_3 = parallel_tuple_cost * path->path.rows;

	path->path.nn_input_width = path->subpath->pathtarget->width;
	path->path.nn_path_rows = path->path.rows;
	path->path.nn_parallel_worker = path->path.parallel_workers;
	...
	inference_ml_cost("Gather", &(path->path), &ml_node_startup_cost, &ml_node_total_cost, &ml_node_startup_std_cost, &ml_node_total_std_cost);
	path->path.ml_node_startup_cost = ml_node_startup_cost;
	path->path.ml_node_total_cost = ml_node_total_cost;
	path->path.ml_node_startup_std_cost = ml_node_startup_std_cost;
	path->path.ml_node_total_std_cost = ml_node_total_std_cost;

	path->path.ml_agg_startup_cost = path->subpath->ml_agg_startup_cost + ml_node_startup_cost;
	path->path.ml_agg_total_cost = path->subpath->ml_agg_total_cost + ml_node_total_cost;
	path->path.ml_agg_startup_std_cost = path->subpath->ml_agg_startup_std_cost + ml_node_startup_std_cost;
	path->path.ml_agg_total_std_cost = path->subpath->ml_agg_total_std_cost + ml_node_total_std_cost;
}
					   ...
					   double *rows)
{
	...
	/* Added by hanjun. */
	/* equations */
	path->path.nn_eq_1 = comparison_cost * N * logN;
	path->path.nn_eq_2 = parallel_setup_cost;
	path->path.nn_eq_3 = path->path.rows * comparison_cost * logN;
	path->path.nn_eq_4 = cpu_operator_cost * path->path.rows;
	path->path.nn_eq_5 = parallel_tuple_cost * path->path.rows * 1.05;

	path->path.nn_input_width = path->path.pathtarget->width;
	path->path.nn_path_rows = path->path.rows;
	path->path.nn_comparison_cost = comparison_cost;
	path->path.nn_num_workers = path->num_workers;
	path->path.nn_parallel_tuple_cost = parallel_tuple_cost;
	...
	inference_ml_cost("Gather Merge", &(path->path), &ml_node_startup_cost, &ml_node_total_cost, &ml_node_startup_std_cost, &ml_node_total_std_cost);
	path->path.ml_node_startup_cost = ml_node_startup_cost;
	path->path.ml_node_total_cost = ml_node_total_cost;
	path->path.ml_node_startup_std_cost = ml_node_startup_std_cost;
	path->path.ml_node_total_std_cost = ml_node_total_std_cost;
				...
				bool partial_path)
{
	...
	double max_pages_fetched;
	double min_pages_fetched;
	...
	/* Added by Hanjun. */
	/* Internal variables used in btcostestimate */
	...
	/* equations */
	/* startup costs */
	path->path.nn_eq_1 = indexStartupCost;
	path->path.nn_eq_2 = qpqual_cost.startup;
	path->path.nn_eq_3 = path->path.pathtarget->cost.startup;
	/* run costs */
	path->path.nn_eq_4 = indexTotalCost - indexStartupCost;
	path->path.nn_eq_5 = max_IO_cost + csquared * (min_IO_cost - max_IO_cost);
	path->path.nn_eq_6 = cpu_per_tuple * tuples_fetched / parallel_divisor;
	path->path.nn_eq_7 = path->path.pathtarget->cost.per_tuple * path->path.rows / parallel_divisor;

	/* Index start
		internal variables used in amcostestimate().
	*/
	path->path.nn_baserel_pages = baserel->pages;
	path->path.nn_baserel_allvisfrac = baserel->allvisfrac;
	path->path.nn_index_pages = index->pages;
	path->path.nn_index_tuples = index->tuples;
	path->path.nn_index_tree_height = index->tree_height;
	path->path.nn_index_ncolumns = index->ncolumns;
	path->path.nn_index_nkeycolumns = index->nkeycolumns;
	path->path.nn_index_unique = index->unique;

	/* genericcostestimate () */
	path->path.nn_index_StartupCost = costs.indexStartupCost;
	path->path.nn_index_indexTotalCost = costs.indexTotalCost;
	path->path.nn_index_numIndexPages = costs.numIndexPages;
	path->path.nn_index_numIndexTuples = costs.numIndexTuples;
	path->path.nn_index_spc_random_page_cost = costs.spc_random_page_cost;
	path->path.nn_index_num_sa_scans = costs.num_sa_scans;

	/* Index end */
	path->path.nn_input_width = path->path.pathtarget->width;
	path->path.nn_pathtarget_cost_per_tuple = path->path.pathtarget->cost.per_tuple;
	path->path.nn_pathtarget_cost_startup = path->path.pathtarget->cost.startup;

	path->path.nn_path_rows = path->path.rows;
	path->path.nn_qp_qual_cost_per_tuple = qpqual_cost.per_tuple;
	path->path.nn_qp_qual_cost_startup = qpqual_cost.startup;

	path->path.nn_indexStartupCost = indexStartupCost;
	path->path.nn_indexTotalCost = indexTotalCost;
	path->path.nn_indexselectivity = indexSelectivity;
	path->path.nn_indexcorrelation = indexCorrelation;
	path->path.nn_indexpage = index_pages;
	path->path.nn_ntuples = baserel->tuples;

	path->path.nn_spc_random_page_cost = spc_random_page_cost;
	path->path.nn_spc_seq_page_cost = spc_seq_page_cost;

	path->path.nn_loopcount = loop_count;
	path->path.nn_match_count = tuples_fetched;
	path->path.nn_max_pages_fetched = max_pages_fetched;
	path->path.nn_min_pages_fetched = min_pages_fetched;
	path->path.nn_max_IO_cost = max_IO_cost;
	path->path.nn_min_IO_cost = min_IO_cost;
	path->path.nn_indexonly = indexonly;
	path->path.nn_partial_path = partial_path;
	path->path.nn_parallel_worker = path->path.parallel_workers;
	...
	inference_ml_cost("Index Scan", &(path->path), &ml_node_startup_cost, &ml_node_total_cost, &ml_node_startup_std_cost, &ml_node_total_std_cost);
	path->path.ml_node_startup_cost = ml_node_startup_cost;
	path->path.ml_node_total_cost = ml_node_total_cost;
	path->path.ml_node_startup_std_cost = ml_node_startup_std_cost;
	path->path.ml_node_total_std_cost = ml_node_total_std_cost;
	...
	/* Base Relation */
	path->path.ml_agg_startup_cost = ml_node_startup_cost;
	path->path.ml_agg_total_cost = ml_node_total_cost;
	path->path.ml_agg_startup_std_cost = ml_node_startup_std_cost;
	path->path.ml_agg_total_std_cost = ml_node_total_std_cost;
}
						   ...
						   Path *bitmapqual, double loop_count)
{
	...
	/* startup costs */
	path->nn_eq_1 = indexTotalCost;
	path->nn_eq_2 = qpqual_cost.startup;
	path->nn_eq_3 = path->pathtarget->cost.startup;
	
	/* run costs */
	path->nn_eq_4 = pages_fetched * cost_per_page;
	path->nn_eq_5 = cpu_per_tuple * tuples_fetched / parallel_divisor;
	path->nn_eq_6 = path->pathtarget->cost.per_tuple * path->rows;

	path->nn_index_StartupCost = indexTotalCost;
	path->nn_pathtarget_cost_startup = path->pathtarget->cost.startup;
	path->nn_pathtarget_cost_per_tuple = path->pathtarget->cost.per_tuple;
	...
	path->nn_path_rows = path->rows;
	path->nn_pages = baserel->pages;
	path->nn_pages_fetched = pages_fetched;
	path->nn_ntuples = tuples_fetched;
	path->nn_spc_random_page_cost = spc_random_page_cost;
	path->nn_spc_seq_page_cost = spc_seq_page_cost;
	path->nn_qp_qual_cost_per_tuple = qpqual_cost.per_tuple;
	path->nn_qp_qual_cost_startup = qpqual_cost.startup;
	path->nn_parallel_worker = path->parallel_workers;
	path->nn_loopcount = loop_count;
	...
	inference_ml_cost("Bitmap Heap Scan", path, &ml_node_startup_cost, &ml_node_total_cost, &ml_node_startup_std_cost, &ml_node_total_std_cost);
	path->ml_node_startup_cost = ml_node_startup_cost;
	path->ml_node_total_cost = ml_node_total_cost;
	path->ml_node_startup_std_cost = ml_node_startup_std_cost;
	path->ml_node_total_std_cost = ml_node_total_std_cost;
 ...
 * 'param_info' is the ParamPathInfo if this is a parameterized path, else NULL
 */
void cost_tidscan(Path *path, PlannerInfo *root,
				  RelOptInfo *baserel, List *tidquals, ParamPathInfo *param_info)
{
	...
	/* equations */
	/* startup costs */
	path->nn_eq_1 = qpqual_cost.startup + tid_qual_cost.per_tuple;
	path->nn_eq_2 = path->pathtarget->cost.startup;
	/* run costs */
	path->nn_eq_3 = spc_random_page_cost * ntuples;
	path->nn_eq_4 = cpu_per_tuple * ntuples;
	path->nn_eq_5 = path->pathtarget->cost.per_tuple * path->rows;

	path->nn_input_width = path->pathtarget->width;
	path->nn_path_rows = path->rows;
	path->nn_ntuples = ntuples;
	path->nn_iscurrentof = isCurrentOf;
	path->nn_per_tuple = path->pathtarget->cost.per_tuple;
	path->nn_qp_qual_cost_per_tuple = qpqual_cost.per_tuple;
	path->nn_qp_qual_cost_startup = qpqual_cost.startup;
	path->nn_scan_qual_cost_startup = tid_qual_cost.startup;
	path->nn_scan_qual_cost_per_tuple = tid_qual_cost.per_tuple;
	path->nn_spc_random_page_cost = spc_random_page_cost;
	...
	inference_ml_cost("Tid Scan", path, &ml_node_startup_cost, &ml_node_total_cost, &ml_node_startup_std_cost, &ml_node_total_std_cost);
	path->ml_node_startup_cost = ml_node_startup_cost;
	path->ml_node_total_cost = ml_node_total_cost;
	path->ml_node_startup_std_cost = ml_node_startup_std_cost;
	path->ml_node_total_std_cost = ml_node_total_std_cost;
 ...
 * 'param_info' is the ParamPathInfo if this is a parameterized path, else NULL
 */
void cost_tidrangescan(Path *path, PlannerInfo *root,
					   ...
					   ParamPathInfo *param_info)
{
	...
	path->nn_eq_2 = path->pathtarget->cost.startup;
	/* run costs */
	path->nn_eq_3 = spc_random_page_cost + spc_seq_page_cost * nseqpages;
	path->nn_eq_4 = cpu_per_tuple * ntuples;
	path->nn_eq_5 = path->pathtarget->cost.per_tuple * path->rows;
	...
	path->nn_ntuples = ntuples;
	path->nn_per_tuple = path->pathtarget->cost.per_tuple;
	path->nn_selectivity = selectivity;
	path->nn_spc_random_page_cost = spc_random_page_cost;
	path->nn_spc_seq_page_cost = spc_seq_page_cost;
	path->nn_qp_qual_cost_per_tuple = qpqual_cost.per_tuple;
	path->nn_qp_qual_cost_startup = qpqual_cost.startup;
	...
	inference_ml_cost("Tid Range Scan", path, &ml_node_startup_cost, &ml_node_total_cost, &ml_node_startup_std_cost, &ml_node_total_std_cost);
	path->ml_node_startup_cost = ml_node_startup_cost;
	path->ml_node_total_cost = ml_node_total_cost;
	path->ml_node_startup_std_cost = ml_node_startup_std_cost;
	path->ml_node_total_std_cost = ml_node_total_std_cost;
					   ...
					   RelOptInfo *baserel, ParamPathInfo *param_info)
{
	...
	/* equations */
	/* startup costs */
	path->path.nn_eq_1 = qpqual_cost.startup;
	path->path.nn_eq_2 = path->path.pathtarget->cost.startup;
	/* run costs */
	path->path.nn_eq_3 = cpu_per_tuple * baserel->tuples;
	path->path.nn_eq_4 = path->path.pathtarget->cost.per_tuple * path->path.rows;
	...
	inference_ml_cost("Subquery Scan", &(path->path), &ml_node_startup_cost, &ml_node_total_cost, &ml_node_startup_std_cost, &ml_node_total_std_cost);
	path->path.ml_node_startup_cost = ml_node_startup_cost;
	path->path.ml_node_total_cost = ml_node_total_cost;
	path->path.ml_node_startup_std_cost = ml_node_startup_std_cost;
	path->path.ml_node_total_std_cost = ml_node_total_std_cost;
 ...
 * 'param_info' is the ParamPathInfo if this is a parameterized path, else NULL
 */
void cost_functionscan(Path *path, PlannerInfo *root,
					   RelOptInfo *baserel, ParamPathInfo *param_info)
{
	Cost startup_cost = 0;
	Cost run_cost = 0;
	...
	/* equations */
	/* startup costs */
	path->nn_eq_1 = exprcost.startup + exprcost.per_tuple;
	path->nn_eq_2 = qpqual_cost.startup;
	path->nn_eq_3 = path->pathtarget->cost.startup;
	/* run costs */
	path->nn_eq_4 = cpu_per_tuple * baserel->tuples;
	...
	path->nn_input_width = path->pathtarget->width;
	path->nn_path_rows = path->rows;
	path->nn_scan_qual_cost_startup = exprcost.startup;
	path->nn_scan_qual_cost_per_tuple = exprcost.per_tuple;
	...
	inference_ml_cost("Function Scan", path, &ml_node_startup_cost, &ml_node_total_cost, &ml_node_startup_std_cost, &ml_node_total_std_cost);
	path->ml_node_startup_cost = ml_node_startup_cost;
	path->ml_node_total_cost = ml_node_total_cost;
	path->ml_node_startup_std_cost = ml_node_startup_std_cost;
	path->ml_node_total_std_cost = ml_node_total_std_cost;
						...
						RelOptInfo *baserel, ParamPathInfo *param_info)
{
	...
	inference_ml_cost("Table Func Scan", path, &ml_node_startup_cost, &ml_node_total_cost, &ml_node_startup_std_cost, &ml_node_total_std_cost);
	path->ml_node_startup_cost = ml_node_startup_cost;
	path->ml_node_total_cost = ml_node_total_cost;
	path->ml_node_startup_std_cost = ml_node_startup_std_cost;
	path->ml_node_total_std_cost = ml_node_total_std_cost;
					 ...
					 RelOptInfo *baserel, ParamPathInfo *param_info)
{
	...
	path->nn_eq_2 = path->pathtarget->cost.startup;
	/* run costs */
	path->nn_eq_3 = cpu_per_tuple * baserel->tuples;
	path->nn_eq_4 = path->pathtarget->cost.per_tuple * path->rows;
	...
	inference_ml_cost("Values Scan", path, &ml_node_startup_cost, &ml_node_total_cost, &ml_node_startup_std_cost, &ml_node_total_std_cost);
	path->ml_node_startup_cost = ml_node_startup_cost;
	path->ml_node_total_cost = ml_node_total_cost;
	path->ml_node_startup_std_cost = ml_node_startup_std_cost;
	path->ml_node_total_std_cost = ml_node_total_std_cost;
 ...
 */
void cost_ctescan(Path *path, PlannerInfo *root,
				  RelOptInfo *baserel, ParamPathInfo *param_info)
{
	Cost startup_cost = 0;
	Cost run_cost = 0;
	...
	inference_ml_cost("Cte Scan", path, &ml_node_startup_cost, &ml_node_total_cost, &ml_node_startup_std_cost, &ml_node_total_std_cost);
	path->ml_node_startup_cost = ml_node_startup_cost;
	path->ml_node_total_cost = ml_node_total_cost;
	path->ml_node_startup_std_cost = ml_node_startup_std_cost;
	path->ml_node_total_std_cost = ml_node_total_std_cost;
 ...
 */
void cost_namedtuplestorescan(Path *path, PlannerInfo *root,
							  RelOptInfo *baserel, ParamPathInfo *param_info)
{
	Cost startup_cost = 0;
	Cost run_cost = 0;
	...
	inference_ml_cost("Named Tuplestore Scan", path, &ml_node_startup_cost, &ml_node_total_cost, &ml_node_startup_std_cost, &ml_node_total_std_cost);
	path->ml_node_startup_cost = ml_node_startup_cost;
	path->ml_node_total_cost = ml_node_total_cost;
	path->ml_node_startup_std_cost = ml_node_startup_std_cost;
	path->ml_node_total_std_cost = ml_node_total_std_cost;
					 ...
					 RelOptInfo *baserel, ParamPathInfo *param_info)
{
	...
	inference_ml_cost("Result Scan", path, &ml_node_startup_cost, &ml_node_total_cost, &ml_node_startup_std_cost, &ml_node_total_std_cost);
						   ...
						   double limit_tuples)
{
	 ...
	 */
	run_cost += group_run_cost + (group_run_cost + group_startup_cost) * (input_groups - 1) + group_input_run_cost * (input_groups - 1);
	...
	/* equations */
	/* startup costs */
	path->nn_eq_1 = group_startup_cost + input_startup_cost + group_input_run_cost;

	/* run costs */
	path->nn_eq_2 = group_run_cost + (group_run_cost + group_startup_cost) * (input_groups - 1) + group_input_run_cost * (input_groups - 1);
	path->nn_eq_3 = (cpu_tuple_cost + comparison_cost) * input_tuples;
	path->nn_eq_4 = 2.0 * cpu_tuple_cost * input_groups;

	path->nn_input_width = path->pathtarget->width;
	path->nn_path_rows = path->rows;
	path->nn_ngroup = input_groups;
	path->nn_ntuples = group_tuples;
	path->nn_group_startup_cost = group_startup_cost;
	path->nn_group_run_cost = group_input_run_cost;
	path->nn_group_input_run_cost = group_run_cost;
	path->nn_comparison_cost = comparison_cost;
	...
	inference_ml_cost("Incremental Sort", path, &ml_node_startup_cost, &ml_node_total_cost, &ml_node_startup_std_cost, &ml_node_total_std_cost);
			   ...
			   double limit_tuples)
{
	...
	/* added by hanjun */
	/* equations */
	path->nn_eq_1 = startup_cost;
	path->nn_eq_2 = run_cost;

	path->nn_input_width = width;
	path->nn_path_rows = tuples;
	...
	inference_ml_cost("Sort", path, &ml_node_startup_cost, &ml_node_total_cost, &ml_node_startup_std_cost, &ml_node_total_std_cost);
	path->ml_node_startup_cost = ml_node_startup_cost;
	path->ml_node_total_cost = ml_node_total_cost;
	path->ml_node_startup_std_cost = ml_node_startup_std_cost;
	path->ml_node_total_std_cost = ml_node_total_std_cost;
	...
	if (!apath->path.parallel_aware)
	{
		...
		if (pathkeys == NIL)
		{
			...
			foreach (l, apath->subpaths)
			{
				...
				/* ML Added */
				apath->path.ml_agg_startup_cost += subpath->ml_agg_startup_cost;
				apath->path.ml_agg_total_cost += subpath->ml_agg_total_cost;
				apath->path.ml_agg_startup_std_cost += subpath->ml_agg_startup_std_cost;
				apath->path.ml_agg_total_std_cost += subpath->ml_agg_total_std_cost;
			}
		}
		else
		{
			...
			foreach (l, apath->subpaths)
			{
				...
				if (!pathkeys_contained_in(pathkeys, subpath->pathkeys))
				{
					...
					/* ML Added */
					apath->path.ml_agg_startup_cost += subpath->ml_node_startup_cost;
					apath->path.ml_agg_total_cost += subpath->ml_node_total_cost;
					apath->path.ml_agg_startup_std_cost += subpath->ml_node_startup_std_cost;
					apath->path.ml_agg_total_std_cost += subpath->ml_node_total_std_cost;
				}
	...
	else /* parallel-aware */
	{
		...
		foreach (l, apath->subpaths)
		{
			...
			if (i == 0){
				...
				/* ML Added */
				apath->path.ml_agg_startup_cost = Min(apath->path.ml_agg_startup_cost, subpath->ml_agg_startup_cost);
				apath->path.ml_agg_startup_std_cost = Max(apath->path.ml_agg_startup_std_cost, subpath->ml_agg_startup_std_cost);
			}
 ...
 */
void cost_merge_append(Path *path, PlannerInfo *root,
					   ...
					   double tuples)
{
	...
	/* equations */
	/* startup costs */
	path->nn_eq_1 = comparison_cost * N * logN;

	/* run costs */
	path->nn_eq_2 = tuples * comparison_cost * logN;
	path->nn_eq_3 = cpu_tuple_cost * APPEND_CPU_COST_MULTIPLIER * tuples;

	path->nn_input_width = path->pathtarget->width;
	path->nn_nstream = n_streams;
	path->nn_ntuples = tuples;
	Cost ml_node_startup_cost = 0.;
	Cost ml_node_total_cost = 0.;
	Cost ml_node_startup_std_cost = 0.;
	Cost ml_node_total_std_cost = 0.;

	inference_ml_cost("Merge Append", path, &ml_node_startup_cost, &ml_node_total_cost, &ml_node_startup_std_cost, &ml_node_total_std_cost);
				   ...
				   double tuples, int width)
{
	...
	path->rows = tuples;
	/* Original code for CPU Computation */
	...
	if (nbytes > work_mem_bytes)
	{
		...
		path->nn_eq_2 = seq_page_cost * npages;
	}

	/* Added by hanjun */
	/* equations */
	/* run costs */
	path->nn_eq_1 = 2 * cpu_operator_cost * tuples;
	...
	inference_ml_cost("Materialize", path, &ml_node_startup_cost, &ml_node_total_cost, &ml_node_startup_std_cost, &ml_node_total_std_cost);
	...
	/* add up agg cost in upper function call */
	/*
	path->ml_agg_startup_cost = input_startup_cost + ml_material_setup_per_tuple_cost * tuples;
	path->ml_agg_total_cost = input_total_cost + ml_material_setup_per_tuple_cost * tuples;
	path->ml_agg_startup_std_cost = ml_node_startup_std_cost;
	path->ml_agg_total_std_cost = ml_node_total_std_cost;
	*/
}
					...
					Cost *rescan_startup_cost, Cost *rescan_total_cost)
{
	 ...
	 * cache.  This is not free. Here we estimate hnct) / ndistinct;

	/*
	 * In order to estimate how costly a single scan wiow often we'll incur the
...
static void
cost_memoize_rescan_ml(PlannerInfo *root, MemoizePath *mpath,
					Cost *rescan_startup_cost, Cost *rescan_total_cost)
{
	EstimationInfo estinfo;
	Cost input_startup_cost = mpath->subpath->ml_agg_startup_cost;
	Cost input_total_cost = mpath->subpath->ml_agg_total_cost;
	double tuples = mpath->subpath->rows;
	double calls = mpath->calls;
	int width = mpath->subpath->pathtarget->width;
	...
	/* Added */
	Cost		time_took_hit_per_call = 0.;
	Cost		time_took_miss_per_call = 0.;
	Cost		ml_node_startup_std_cost = 0.;
	Cost		ml_node_total_std_cost = 0.;

	mpath->path.nn_path_rows = tuples;
	mpath->path.nn_input_width = width;
	inference_ml_cost("Memoize", &(mpath->path), &time_took_hit_per_call, &time_took_miss_per_call, &ml_node_startup_std_cost, &ml_node_total_std_cost);

	double hits   = calls * hit_ratio;
	double misses = calls * (1.0 - hit_ratio);

	*rescan_startup_cost = input_startup_cost * (1.0 - hit_ratio);
	*rescan_total_cost = time_took_hit_per_call * hits + time_took_miss_per_call * misses + misses * (input_total_cost  - input_startup_cost);
}
			  ...
			  double input_tuples, double input_width)
{
	...
	path->nn_eq_1 = aggcosts->transCost.startup;
	path->nn_eq_2 = aggcosts->transCost.per_tuple * input_tuples;
	path->nn_eq_3 = aggcosts->finalCost.startup;
	...
	if (aggstrategy == AGG_PLAIN)
	{
		...
		path->nn_eq_4 = aggcosts->finalCost.per_tuple;
	}
	else if (aggstrategy == AGG_SORTED || aggstrategy == AGG_MIXED)
	{
		...
		path->nn_eq_5 = (cpu_operator_cost * numGroupCols) * input_tuples;
		path->nn_eq_6 = aggcosts->finalCost.per_tuple * numGroups;
		path->nn_eq_7 = cpu_tuple_cost * numGroups;
	}
	else
	{
		/* must be AGG_HASHED */
	...
	if (aggstrategy == AGG_HASHED || aggstrategy == AGG_MIXED)
	{
		...
		path->nn_eq_8 = pages_written * random_page_cost;
		path->nn_eq_9 = pages_read * seq_page_cost;
		path->nn_eq_10 = spill_cost;
	}
	...
	if (quals)
	{
		...
		path->nn_eq_11 = qual_cost.startup + output_tuples * qual_cost.per_tuple;
	...
	path->nn_input_width = path->pathtarget->width;
	path->nn_path_rows = path->rows;
	path->nn_aggstrategy = aggstrategy;
	path->nn_ntuples = input_tuples;
	path->nn_agg_transcost_startup = aggcosts->transCost.startup;
	path->nn_agg_transcost_per_tuple = aggcosts->transCost.per_tuple;
	path->nn_agg_finalcost_startup = aggcosts->finalCost.startup;
	path->nn_agg_finalcost_per_tuple = aggcosts->finalCost.per_tuple;
	path->nn_ngroups = numGroups;
	path->nn_ngroupcols = numGroupCols;
	path->nn_input_width = input_width;
	path->nn_hashentrysize = hashentrysize;
	path->nn_ngroups_limit = ngroups_limit;
	path->nn_num_partitions = num_partitions;
	path->nn_nbatches = nbatches;
	path->nn_depth = depth;
	path->nn_pages = pages;
	path->nn_pages_written = pages_written;
	path->nn_pages_read = pages_read;
	path->nn_qp_qual_cost_per_tuple = qual_cost.per_tuple;
	path->nn_qp_qual_cost_startup = qual_cost.startup;
	...
	inference_ml_cost("Aggregate", path, &ml_node_startup_cost, &ml_node_total_cost, &ml_node_startup_std_cost, &ml_node_total_std_cost);
	...
	/*
	we add up further in the outer function call.
	...
	*/
	...
	else if (aggstrategy == AGG_SORTED || aggstrategy == AGG_MIXED)
	{
		if (aggstrategy == AGG_MIXED && !enable_hashagg)
		{
			path->ml_node_startup_cost += disable_cost;
			path->ml_node_total_cost += disable_cost;
		}
	}
	else
	{
 ...
 */
void cost_windowagg(Path *path, PlannerInfo *root,
					...
					double input_tuples)
{
	...
	path->nn_input_width = path->pathtarget->width;
	path->nn_path_rows = path->rows;
	path->nn_numpartcols = numPartCols;
	path->nn_numordercols = numOrderCols;
	...
	inference_ml_cost("Window Agg", path, &ml_node_startup_cost, &ml_node_total_cost, &ml_node_startup_std_cost, &ml_node_total_std_cost);
 ...
 * input.
 */
void cost_group(Path *path, PlannerInfo *root,
				int numGroupCols, double numGroups,
				List *quals,
				Cost input_startup_cost, Cost input_total_cost,
				double input_tuples)
{
	...
	if (quals)
	{
		...
		/* equations */
		/* startup costs */
		path->nn_eq_1 = qual_cost.startup;

		/* run costs */
		path->nn_eq_2 = qual_cost.startup + output_tuples * qual_cost.per_tuple;
	}

	path->rows = output_tuples;

	/* Added by hanjun */
	path->nn_eq_3 = cpu_operator_cost * input_tuples * numGroupCols;

	path->nn_input_width = path->pathtarget->width;
	path->nn_path_rows = path->rows;
	path->nn_numgroupcols = numGroupCols;
	path->nn_qp_qual_cost_per_tuple = qual_cost.per_tuple;
	path->nn_qp_qual_cost_startup = qual_cost.startup;
	...
	inference_ml_cost("Group", path, &ml_node_startup_cost, &ml_node_total_cost, &ml_node_startup_std_cost, &ml_node_total_std_cost);
						   ...
						   JoinPathExtraData *extra)
{
	...
	Cost ml_startup_cost = 0;
	Cost ml_run_cost = 0;
	Cost ml_inner_rescan_start_cost;
	Cost ml_inner_rescan_total_cost;
	Cost ml_inner_run_cost;
	Cost ml_inner_rescan_run_cost;

	cost_rescan_ml(root, inner_path,
				&ml_inner_rescan_start_cost,
				&ml_inner_rescan_total_cost);
	...
	ml_startup_cost += outer_path->ml_agg_startup_cost + inner_path->ml_agg_startup_cost;
	ml_run_cost += outer_path->ml_agg_total_cost - outer_path->ml_agg_startup_cost;
	...
	if (outer_path_rows > 1)
	{
		...
		/* Added */
		ml_run_cost += (outer_path_rows - 1) * ml_inner_rescan_start_cost;
		workspace->nn_eq_1 = (outer_path_rows - 1) * inner_rescan_start_cost;
	}
	...
	/* Added */
	ml_inner_run_cost = inner_path->ml_agg_total_cost - inner_path->ml_agg_startup_cost;
	ml_inner_rescan_run_cost = ml_inner_rescan_total_cost - ml_inner_rescan_start_cost;
	
	workspace->ml_inner_rescan_start_cost = ml_inner_rescan_start_cost;
	workspace->ml_inner_run_cost = ml_inner_run_cost;
	workspace->ml_inner_rescan_run_cost = ml_inner_rescan_run_cost;
	...
	else
	{
		...
		ml_run_cost += ml_inner_run_cost;
		workspace->nn_eq_2 = inner_run_cost;
		if (outer_path_rows > 1)
		{
			run_cost += (outer_path_rows - 1) * inner_rescan_run_cost;
			ml_run_cost += (outer_path_rows - 1) * ml_inner_rescan_run_cost;
			workspace->nn_eq_3 = (outer_path_rows - 1) * inner_rescan_run_cost;
		}
	}
	...
	/* Added */
	workspace->ml_startup_cost = ml_startup_cost;
	workspace->ml_run_cost = ml_run_cost;
	workspace->ml_total_cost = ml_startup_cost + ml_run_cost;
}
						 ...
						 JoinPathExtraData *extra)
{
	...
	Cost ml_startup_cost = workspace->ml_startup_cost;
	Cost ml_run_cost = workspace->ml_run_cost;

	Cost cpu_per_tuple = 0.;
	QualCost restrict_qual_cost;
	double ntuples = 0.;
	restrict_qual_cost.per_tuple = 0.;
	restrict_qual_cost.startup = 0.;
	...
	Cost inner_run_cost = 0.;
	Cost inner_rescan_run_cost = 0.;
	double outer_matched_rows = 0.;
	double outer_unmatched_rows = 0.;
	Selectivity inner_scan_frac = 0.;
	int match_count = extra->semifactors.match_count;
	bool bool_has_indexed_join_quals = has_indexed_join_quals(path);

	Cost inner_rescan_start_cost = workspace->inner_rescan_start_cost;
	inner_run_cost = workspace->inner_run_cost;
	inner_rescan_run_cost = workspace->inner_rescan_run_cost;
	
	Cost ml_inner_rescan_start_cost = workspace->ml_inner_rescan_start_cost;
	Cost ml_inner_run_cost = workspace->ml_inner_run_cost;
	Cost ml_inner_rescan_run_cost = workspace->ml_inner_rescan_run_cost;
		...
		extra->inner_unique)
	{
		...
		if (has_indexed_join_quals(path))
		{
			...
			ml_run_cost += ml_inner_run_cost * inner_scan_frac;
			path->path.nn_eq_6 = inner_run_cost * inner_scan_frac;

			if (outer_matched_rows > 1)
			{
				run_cost += (outer_matched_rows - 1) * inner_rescan_run_cost * inner_scan_frac;
				
				ml_run_cost += (outer_matched_rows - 1) * ml_inner_rescan_run_cost * inner_scan_frac;
				path->path.nn_eq_7 = (outer_matched_rows - 1) * inner_rescan_run_cost * inner_scan_frac;
			}
			...
			ml_run_cost += outer_unmatched_rows * ml_inner_rescan_run_cost / inner_path_rows;
			path->path.nn_eq_8 = outer_unmatched_rows * inner_rescan_run_cost / inner_path_rows;
			/*
		...
		else
		{
			...
			ml_run_cost += ml_inner_run_cost;
			path->path.nn_eq_9 = inner_run_cost;
			...
			if (outer_matched_rows > 0)
			{
				...
				ml_run_cost += outer_matched_rows * ml_inner_rescan_run_cost * inner_scan_frac;
				path->path.nn_eq_10 = outer_matched_rows * inner_rescan_run_cost * inner_scan_frac;
			}
			...
			if (outer_unmatched_rows > 0)
			{
				...
				ml_run_cost += outer_unmatched_rows * ml_inner_rescan_run_cost;
				path->path.nn_eq_11 = outer_unmatched_rows * inner_rescan_run_cost;
			}
		}
	}
	else
	...
	run_cost += cpu_per_tuple * ntuples;
	path->path.nn_eq_12 = cpu_per_tuple * ntuples;

	/* tlist eval costs are paid per output row, not per tuple scanned */
	startup_cost += path->path.pathtarget->cost.startup;
	run_cost += path->path.pathtarget->cost.per_tuple * path->path.rows;
	path->path.nn_eq_13 = path->path.pathtarget->cost.per_tuple * path->path.rows;

	path->path.startup_cost = startup_cost;
	path->path.total_cost = startup_cost + run_cost;
	
	/* Added by hanjun */
	/* equations from workspace */
	path->path.ml_inner_run_cost = inner_run_cost;
	path->path.ml_inner_rescan_run_cost = inner_rescan_run_cost;
	path->path.ml_inner_rescan_start_cost = inner_rescan_start_cost;
	
	path->path.nn_eq_1 = workspace->nn_eq_1;
	path->path.nn_eq_2 = workspace->nn_eq_2;
	path->path.nn_eq_3 = workspace->nn_eq_3;
	path->path.nn_eq_4 = restrict_qual_cost.startup;
	path->path.nn_eq_5 = path->path.pathtarget->cost.startup;
	/* run costs eq_6 ~ eq_13 */

	path->path.nn_restrict_qual_cost_startup = restrict_qual_cost.startup;
	path->path.nn_pathtarget_cost_startup = path->path.pathtarget->cost.startup;
	path->path.nn_pathtarget_cost_per_tuple = path->path.pathtarget->cost.per_tuple;

	path->path.nn_input_width = path->path.pathtarget->width;
	path->path.nn_outer_path_rows = outer_path_rows;
	path->path.nn_inner_path_rows = inner_path_rows;
	path->path.nn_path_rows = path->path.rows;
	path->path.nn_ntuples = ntuples;
	path->path.nn_parallel_worker = path->path.parallel_workers;
	path->path.nn_per_tuple = restrict_qual_cost.per_tuple;
	path->path.nn_jointype = path->jointype;
	path->path.nn_outer_matched_rows = outer_matched_rows;
	path->path.nn_inner_unique = extra->inner_unique;
	path->path.nn_has_indexed_join_quals = bool_has_indexed_join_quals;
	path->path.nn_match_count = match_count;
	path->path.nn_inner_scan_frac = inner_scan_frac;
	...
	inference_ml_cost("Nested Loop", &(path->path), &ml_node_startup_cost, &ml_node_total_cost, &ml_node_startup_std_cost, &ml_node_total_std_cost);
	path->path.ml_node_startup_cost = ml_node_startup_cost;
	path->path.ml_node_total_cost = ml_node_total_cost;
	path->path.ml_node_startup_std_cost = ml_node_startup_std_cost;
	path->path.ml_node_total_std_cost = ml_node_total_std_cost;

	path->path.ml_agg_startup_cost = ml_startup_cost; + ml_node_startup_cost;
	path->path.ml_agg_total_cost = ml_startup_cost + ml_run_cost + ml_node_total_cost;
	/* repeat inner_run_cost (outer_path_rows-1) times */
	/* Need to consider rescan */
	/* Outdated 
	path->path.ml_agg_startup_cost = outer_path->ml_agg_startup_cost + inner_path->ml_agg_startup_cost + ml_node_startup_cost;
	path->path.ml_agg_total_cost = outer_path->ml_agg_total_cost + inner_path->ml_agg_total_cost + ml_node_total_cost;
	path->path.ml_agg_total_cost += (outer_path_rows - 1) * (inner_path->ml_agg_total_cost - inner_path->ml_agg_startup_cost);
	*/
	path->path.ml_agg_startup_std_cost = outer_path->ml_agg_startup_std_cost + inner_path->ml_agg_startup_std_cost + ml_node_startup_std_cost;
	path->path.ml_agg_total_std_cost = outer_path->ml_agg_total_std_cost + inner_path->ml_agg_total_std_cost + ml_node_total_std_cost;

	if (!enable_nestloop){
		path->path.ml_agg_startup_cost += disable_cost;
		path->path.ml_agg_total_cost += disable_cost;
	}
}
							...
							JoinPathExtraData *extra)
{
	...
	if (outersortkeys) /* do we need to sort outer? */
	{
		...
		ml_startup_cost += outer_path->ml_agg_startup_cost + sort_path.ml_node_startup_cost;
		ml_startup_cost += (sort_path.ml_node_total_cost - sort_path.ml_node_startup_cost) * outerstartsel;
		ml_run_cost += (sort_path.ml_node_total_cost - sort_path.ml_node_startup_cost) * (outerendsel - outerstartsel);

		workspace->nn_eq_1 = sort_path.startup_cost;
		workspace->nn_eq_2 = (sort_path.total_cost - sort_path.startup_cost) * outerstartsel;
		workspace->nn_eq_3 = (sort_path.total_cost - sort_path.startup_cost) * (outerendsel - outerstartsel);

		/* Save private data for final_cost_mergejoin */
		workspace->nn_sort_left = true;
		workspace->ml_sort_left_cost = sort_path.ml_node_total_cost;
		workspace->ml_sort_left_std_cost = sort_path.ml_node_total_std_cost;
	}
	else
	{
		...
		ml_startup_cost += outer_path->ml_agg_startup_cost;
		ml_startup_cost += (outer_path->ml_agg_total_cost - outer_path->ml_agg_startup_cost) * outerstartsel;
		ml_run_cost += (outer_path->ml_agg_total_cost - outer_path->ml_agg_startup_cost) * (outerendsel - outerstartsel);

		workspace->nn_eq_1 = outer_path->startup_cost;
		workspace->nn_eq_2 = (outer_path->total_cost - outer_path->startup_cost) * outerstartsel;
		workspace->nn_eq_3 = (outer_path->total_cost - outer_path->startup_cost) * (outerendsel - outerstartsel);

		/* Save private data for final_cost_mergejoin */
		workspace->nn_sort_left = false;
		workspace->ml_sort_left_cost = 0.;
		workspace->ml_sort_left_std_cost = 0.;
	}
	...
	if (innersortkeys) /* do we need to sort inner? */
	{
		...
		ml_startup_cost += inner_path->ml_agg_startup_cost + sort_path.ml_node_startup_cost;
		ml_startup_cost += (sort_path.ml_node_total_cost - sort_path.ml_node_startup_cost) * innerstartsel;
		ml_inner_run_cost += (sort_path.ml_node_total_cost - sort_path.ml_node_startup_cost) * (innerendsel - innerstartsel);

		workspace->nn_eq_4 = sort_path.startup_cost;
		workspace->nn_eq_5 = (sort_path.total_cost - sort_path.startup_cost) * innerstartsel;
		workspace->nn_eq_6 = (sort_path.total_cost - sort_path.startup_cost) * (innerendsel - innerstartsel);

		/* Save private data for final_cost_mergejoin */
		workspace->nn_sort_right = true;
		workspace->ml_sort_right_cost = sort_path.ml_node_total_cost;
		workspace->ml_sort_right_std_cost = sort_path.ml_node_total_std_cost;
	}
	else
	{
		...
		ml_startup_cost += inner_path->ml_agg_startup_cost;
		ml_startup_cost += (inner_path->ml_agg_total_cost - inner_path->ml_agg_startup_cost) * innerstartsel;
		ml_inner_run_cost += (inner_path->ml_agg_total_cost - inner_path->ml_agg_startup_cost) * (innerendsel - innerstartsel);

		workspace->nn_eq_4 = inner_path->startup_cost;
		workspace->nn_eq_5 = (inner_path->total_cost - inner_path->startup_cost) * innerstartsel;
		workspace->nn_eq_6 = (inner_path->total_cost - inner_path->startup_cost) * (innerendsel - innerstartsel);

		/* Save private data for final_cost_mergejoin */
		workspace->nn_sort_right = false;
		workspace->ml_sort_right_cost = 0.;
		workspace->ml_sort_right_std_cost = 0.;
	}
	...
	workspace->ml_startup_cost = ml_startup_cost;
	workspace->ml_total_cost = ml_startup_cost + ml_run_cost + ml_inner_run_cost;
	workspace->ml_run_cost = ml_run_cost;
	workspace->ml_inner_run_cost = ml_inner_run_cost;
						  ...
						  JoinPathExtraData *extra)
{
	...
	Cost ml_run_cost = workspace->ml_run_cost;
	Cost ml_inner_run_cost = workspace->ml_inner_run_cost;
	Cost ml_bare_inner_cost = 0.;
	Cost ml_mat_inner_cost = 0.;
	...
	ml_bare_inner_cost = ml_inner_run_cost * rescanratio;
	...
	/* Added by hanjun */
	path->jpath.path.nn_path_rows = inner_rows;
	path->jpath.path.nn_input_width = inner_path->pathtarget->width;

	mat_inner_cost = inner_run_cost + cpu_operator_cost * inner_rows * rescanratio;
	
	Cost		time_took_hit_per_row = 0.;
	Cost		time_took_miss_per_row = 0.;
	Cost		ml_std_startup_cost_placeholder = 0.;
	Cost		ml_std_total_cost_placeholder = 0.;
	inference_ml_cost("Materialize Lookup", path, &time_took_hit_per_row, &time_took_miss_per_row, &ml_std_startup_cost_placeholder, &ml_std_total_cost_placeholder);
	ml_mat_inner_cost = ml_inner_run_cost + time_took_hit_per_row * inner_rows * rescanratio;
	
	bool less_material_cost = mat_inner_cost < bare_inner_cost;
	if (use_cost_ml_server)
		less_material_cost = ml_mat_inner_cost < ml_bare_inner_cost;
	 ...
	 */
	else if (enable_material && less_material_cost)
	...
	path->jpath.path.ml_inner_run_cost = inner_run_cost;
	if (path->materialize_inner)
	{
		run_cost += mat_inner_cost;
		ml_run_cost += ml_mat_inner_cost;
		path->jpath.path.ml_inner_rescan_run_cost = mat_inner_cost;
		path->jpath.path.nn_eq_11 = mat_inner_cost;
	}
	else
	{
		run_cost += bare_inner_cost;
		ml_run_cost += ml_bare_inner_cost;
		path->jpath.path.ml_inner_rescan_run_cost = bare_inner_cost;
		path->jpath.path.nn_eq_11 = bare_inner_cost;
	}
	...
	/* Added by hanjun */
	/* equations in initial_ */
	path->jpath.path.nn_eq_1 = workspace->nn_eq_1;
	path->jpath.path.nn_eq_2 = workspace->nn_eq_2;
	path->jpath.path.nn_eq_3 = workspace->nn_eq_3;
	path->jpath.path.nn_eq_4 = workspace->nn_eq_4;
	path->jpath.path.nn_eq_5 = workspace->nn_eq_5;
	path->jpath.path.nn_eq_6 = workspace->nn_eq_6;

	/* equations in final_, startup_cost */
	path->jpath.path.nn_eq_7 = merge_qual_cost.startup;
	path->jpath.path.nn_eq_8 = merge_qual_cost.per_tuple * (outer_skip_rows + inner_skip_rows * rescanratio);
	path->jpath.path.nn_eq_9 = qp_qual_cost.startup;
	path->jpath.path.nn_eq_10 = path->jpath.path.pathtarget->cost.startup;

	/* equations in final_, run_cost */
	path->jpath.path.nn_eq_12 = merge_qual_cost.per_tuple *
								((outer_rows - outer_skip_rows) + (inner_rows - inner_skip_rows) * rescanratio);
	path->jpath.path.nn_eq_13 = cpu_per_tuple * mergejointuples;
	path->jpath.path.nn_eq_14 = path->jpath.path.pathtarget->cost.per_tuple * path->jpath.path.rows;

	path->jpath.path.nn_sort_left = workspace->nn_sort_left;
	path->jpath.path.ml_sort_left_cost = workspace->ml_sort_left_cost;
	path->jpath.path.ml_sort_left_std_cost = workspace->ml_sort_left_std_cost;

	path->jpath.path.nn_sort_right = workspace->nn_sort_right;
	path->jpath.path.ml_sort_right_cost = workspace->ml_sort_right_cost;
	path->jpath.path.ml_sort_right_std_cost = workspace->ml_sort_right_std_cost;

	path->jpath.path.nn_input_width = path->jpath.path.pathtarget->width;
	path->jpath.path.nn_parallel_worker = path->jpath.path.parallel_workers;
	path->jpath.path.nn_inner_path_rows = inner_path_rows;
	path->jpath.path.nn_path_rows = path->jpath.path.rows;
	path->jpath.path.nn_jointype = path->jpath.jointype;
	path->jpath.path.nn_inner_unique = extra->inner_unique;

	path->jpath.path.nn_outer_rows = outer_rows;
	path->jpath.path.nn_inner_rows = inner_rows;
	path->jpath.path.nn_outer_skip_rows = outer_skip_rows;
	path->jpath.path.nn_inner_skip_rows = inner_skip_rows;

	path->jpath.path.nn_rescanratio = rescanratio;

	path->jpath.path.nn_outerstartsel = workspace->outerstartsel;
	path->jpath.path.nn_outerendsel = workspace->outerendsel;
	path->jpath.path.nn_innerstartsel = workspace->innerstartsel;
	path->jpath.path.nn_innerendsel = workspace->innerendsel;

	path->jpath.path.nn_merge_qual_cost_startup = merge_qual_cost.startup;
	path->jpath.path.nn_merge_qual_cost_per_tuple = merge_qual_cost.per_tuple;
	path->jpath.path.nn_qp_qual_cost_startup = qp_qual_cost.startup;
	path->jpath.path.nn_qp_qual_cost_per_tuple = qp_qual_cost.per_tuple;

	path->jpath.path.nn_pathtarget_cost_startup = path->jpath.path.pathtarget->cost.startup;
	path->jpath.path.nn_pathtarget_cost_per_tuple = path->jpath.path.pathtarget->cost.per_tuple;

	path->jpath.path.nn_mergejointuples = mergejointuples;
	path->jpath.path.nn_rescannedtuples = rescannedtuples;

	path->jpath.path.nn_materialize_inner = path->materialize_inner;
	path->jpath.path.nn_skip_mark_restore = path->skip_mark_restore;
	...
	inference_ml_cost("Merge Join", &(path->jpath.path), &ml_node_startup_cost, &ml_node_total_cost, &ml_node_startup_std_cost, &ml_node_total_std_cost);
	path->jpath.path.ml_node_startup_cost = ml_node_startup_cost;
	path->jpath.path.ml_node_total_cost = ml_node_total_cost;
	path->jpath.path.ml_node_startup_std_cost = ml_node_startup_std_cost;
	path->jpath.path.ml_node_total_std_cost = ml_node_total_std_cost;

	/* outer + inner + current */
	path->jpath.path.ml_agg_startup_cost = ml_startup_cost + ml_node_startup_cost;
	path->jpath.path.ml_agg_total_cost = ml_startup_cost + ml_run_cost + ml_node_total_cost;

	path->jpath.path.ml_agg_startup_std_cost = outer_path->ml_agg_startup_std_cost + inner_path->ml_agg_startup_std_cost + ml_node_startup_std_cost;
	path->jpath.path.ml_agg_total_std_cost = outer_path->ml_agg_total_std_cost + inner_path->ml_agg_total_std_cost + ml_node_total_std_cost;

	if (!enable_mergejoin) {
		path->jpath.path.ml_agg_startup_cost += disable_cost;
		path->jpath.path.ml_agg_total_cost += disable_cost;
	}
}
						   ...
						   bool parallel_hash)
{
	...
	workspace->nn_eq_1 = (cpu_operator_cost * num_hashclauses + cpu_tuple_cost) * inner_path_rows;
	workspace->nn_eq_2 = cpu_operator_cost * num_hashclauses * outer_path_rows;
	...
	if (numbatches > 1)
	{
		...
		workspace->nn_eq_3 = seq_page_cost * innerpages;
		workspace->nn_eq_4 = seq_page_cost * (innerpages + 2 * outerpages);
	}
						 ...
						 JoinPathExtraData *extra)
{
		...
		extra->inner_unique)
	{
		...
		path->jpath.path.nn_eq_10 = hash_qual_cost.per_tuple * outer_matched_rows *
									clamp_row_est(inner_path_rows * innerbucketsize * inner_scan_frac) * 0.5;
		path->jpath.path.nn_eq_11 = hash_qual_cost.per_tuple *
									(outer_path_rows - outer_matched_rows) *
									clamp_row_est(inner_path_rows / virtualbuckets) * 0.05;
	...
	else
	{
		...
		path->jpath.path.nn_eq_12 = hash_qual_cost.per_tuple * outer_path_rows *
									clamp_row_est(inner_path_rows * innerbucketsize) * 0.5;
	...
	path->jpath.path.nn_eq_3 = workspace->nn_eq_3;
	path->jpath.path.nn_eq_4 = workspace->nn_eq_4;
	/* equations for startup in final */
	path->jpath.path.nn_eq_5 = hash_qual_cost.startup;
	path->jpath.path.nn_eq_6 = qp_qual_cost.startup;
	path->jpath.path.nn_eq_7 = path->jpath.path.pathtarget->cost.startup;
	/* equations for total in final */
	path->jpath.path.nn_eq_8 = cpu_per_tuple * hashjointuples;
	path->jpath.path.nn_eq_9 = path->jpath.path.pathtarget->cost.per_tuple * path->jpath.path.rows;
	/* und 10 ~ 12 are in the code */

	path->jpath.path.nn_input_width = path->jpath.path.pathtarget->width;
	path->jpath.path.nn_path_rows = path->jpath.path.rows;
	path->jpath.path.nn_parallel_worker = path->jpath.path.parallel_workers;
	path->jpath.path.nn_numbatches = numbatches;
	path->jpath.path.nn_num_hashclauses = workspace->num_hashclauses;
	path->jpath.path.nn_outerpages = outerpages;
	path->jpath.path.nn_innerpages = innerpages;

	path->jpath.path.nn_inner_rows = path->inner_rows_total;
	path->jpath.path.nn_virtualbuckets = virtualbuckets;
	path->jpath.path.nn_innerbucketsize = innerbucketsize;
	path->jpath.path.nn_innermcvfreq = innermcvfreq;

	path->jpath.path.nn_qp_qual_cost_startup = qp_qual_cost.startup;
	path->jpath.path.nn_qp_qual_cost_per_tuple = qp_qual_cost.per_tuple;
	path->jpath.path.nn_hash_qual_cost_startup = hash_qual_cost.startup;
	path->jpath.path.nn_hash_qual_cost_per_tuple = hash_qual_cost.per_tuple;

	path->jpath.path.nn_jointype = path->jpath.jointype;
	path->jpath.path.nn_outer_matched_rows = outer_matched_rows;
	path->jpath.path.nn_inner_scan_frac = inner_scan_frac;
	path->jpath.path.nn_inner_path_rows = inner_path_rows;
	path->jpath.path.nn_outer_path_rows = outer_path_rows;

	path->jpath.path.nn_hashjointuples = hashjointuples;
	path->jpath.path.nn_inner_unique = extra->inner_unique;
	path->jpath.path.nn_pathtarget_cost_startup = path->jpath.path.pathtarget->cost.startup;
	path->jpath.path.nn_pathtarget_cost_per_tuple = path->jpath.path.pathtarget->cost.per_tuple;
	...
	/*  Assign some internal parameters in inner_path for Hash node */
	Cost ml_node_hash_startup_cost = 0.;
	Cost ml_node_hash_total_cost = 0.;
	Cost ml_node_hash_startup_std_cost = 0.;
	Cost ml_node_hash_total_std_cost = 0.;
	
	inner_path->nn_num_hashclauses = workspace->num_hashclauses;
	inner_path->nn_innerpages = innerpages;
	inner_path->nn_numbatches = numbatches;
	inner_path->nn_inner_rows = path->inner_rows_total;
	inner_path->nn_virtualbuckets = virtualbuckets;
	inner_path->nn_innerbucketsize = innerbucketsize;
	inner_path->nn_innermcvfreq = innermcvfreq;
	inference_ml_cost("Hash", inner_path, &ml_node_hash_startup_cost, &ml_node_hash_total_cost, &ml_node_hash_startup_std_cost, &ml_node_hash_total_std_cost);
	
	Cost ml_node_startup_cost = 0.;
	Cost ml_node_total_cost = 0.;
	Cost ml_node_startup_std_cost = 0.;
	Cost ml_node_total_std_cost = 0.;
	inference_ml_cost("Hash Join", &(path->jpath.path), &ml_node_startup_cost, &ml_node_total_cost, &ml_node_startup_std_cost, &ml_node_total_std_cost);
	
	path->jpath.path.ml_node_startup_cost = ml_node_hash_total_cost + ml_node_startup_cost;
	path->jpath.path.ml_node_total_cost = ml_node_hash_total_cost + ml_node_total_cost;
	path->jpath.path.ml_node_startup_std_cost = ml_node_startup_std_cost;
	path->jpath.path.ml_node_total_std_cost = ml_node_total_std_cost;

	path->jpath.path.ml_agg_startup_cost = outer_path->ml_agg_startup_cost + inner_path->ml_agg_total_cost + ml_node_startup_cost;
	path->jpath.path.ml_agg_total_cost = outer_path->ml_agg_total_cost + inner_path->ml_agg_total_cost + ml_node_total_cost;
	path->jpath.path.ml_agg_startup_std_cost = outer_path->ml_agg_startup_std_cost + inner_path->ml_agg_total_std_cost + ml_node_startup_std_cost;
	path->jpath.path.ml_agg_total_std_cost = outer_path->ml_agg_total_std_cost + inner_path->ml_agg_total_std_cost + ml_node_total_std_cost;

	if (!enable_hashjoin)
	{
		path->jpath.path.startup_cost += disable_cost;
		path->jpath.path.total_cost += disable_cost;
	}
}
...
void cost_subplan(PlannerInfo *root, SubPlan *subplan, Plan *plan)
{
	...
	if (use_cost_ml_server){
		Cost ml_startup = plan->ml_node_startup_cost;
        Cost ml_total   = plan->ml_node_total_cost;

        double rows = clamp_row_est(plan->plan_rows);
        sp_cost.startup = ml_startup;
        sp_cost.per_tuple = (rows > 0) ?
            (ml_total - ml_startup) / rows : 0;
...
static void
cost_rescan_ml(PlannerInfo *root, Path *path,
			Cost *rescan_startup_cost, /* output parameters */
			Cost *rescan_total_cost)
{
	if (!use_cost_ml_server){
	...
	case T_FunctionScan:
		 ...
		 */
		*rescan_startup_cost = 0;
		*rescan_total_cost = path->ml_agg_total_cost - path->ml_agg_startup_cost;
		break;
	case T_HashJoin:
		/*
		 * If it's a single-batch join, we don't need to rebuild the hash
		...
		{
			/* Otherwise, no special treatment */
			*rescan_startup_cost = path->ml_agg_startup_cost;
			*rescan_total_cost = path->ml_agg_total_cost;
		}
		break;
	case T_CteScan:
	case T_WorkTableScan:
	{
		...
		inference_ml_cost("Materialize Lookup", path, &time_took_hit_per_row, &time_took_miss_per_row, &ml_node_startup_std_cost, &ml_node_total_std_cost);

		*rescan_startup_cost = 0.;
		*rescan_total_cost = path->rows * time_took_hit_per_row;
	}
	break;
	case T_Material:
	case T_Sort:
	...
	case T_Memoize:
		/* All the hard work is done by cost_memoize_rescan */
		cost_memoize_rescan_ml(root, (MemoizePath *)path,
							rescan_startup_cost, rescan_total_cost);
		break;
	default:
		*rescan_startup_cost = path->ml_agg_startup_cost;
						   ...
						   List *restrictlist_in)
{
	...
	if (ml_joinest_enabled)
	{
		...
		join_est = join_card_ests[join_est_no];
